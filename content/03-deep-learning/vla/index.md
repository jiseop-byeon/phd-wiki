...
*In one sentence:* the action head turns the policy's features into the numbers the robot executes, and its two families pay differently — a token head decodes one number at a time, a denoiser pays a fixed number of passes for the whole chunk — so the choice follows the chunk length and the control budget.
...
D4가 $(0.02,0)$을 미터로 세 번 예측했는데 하위 interface가 센티미터로 해석하면, 의도한 6 cm가 0.06 cm가 된다. frame과 rate도 같은 식으로 깨진다. D4의 변화량은 base frame에서 쓴 것이다. 이것을 tool frame에서 변화량을 읽는 interface에 보내고 공구가 작업 평면 안에서 $90^\circ$ 돌아가 있으면, 변화량마다 가는 길에 회전되어 팔은 $(0,-0.02)$, $(0,-0.02)$, $(0.02,0)$을 실행하고 $(0.02,-0.04)\,\mathrm{m}$에서 멈춘다. 목표에서 $0.0632\,\mathrm{m}$ 떨어진 곳으로, 출발점의 $0.0447\,\mathrm{m}$보다 더 멀다. 하나의 운동이 두 frame에서 두 가지로 기술되는 방식은 [[04-robotics/modern-robotics/ch03-rigid-body-motions|MR 3장 §4]]에 있다. 또 변화량 하나는 $\Delta t=0.05\,\mathrm{s}$당 $0.02\,\mathrm{m}$, 곧 공구 속도 $0.4\,\mathrm{m/s}$를 뜻한다. 같은 chunk를 $50\,\mathrm{Hz}$ interface에서 재생하면 공구는 맞는 경로를 $1.0\,\mathrm{m/s}$로, 두 배 반 빠르게, $0.15\,\mathrm{s}$가 아니라 $0.06\,\mathrm{s}$에 지나간다. 표현 오차는 물리 오차다. 여러 로봇의 데이터를 모으면 같은 종류의 오차가 체계적이 되고, §7이 그것을 잰다.
...
*한 문장으로:* 행동 헤드는 정책의 특징을 로봇이 실행할 숫자로 바꾸고, 두 계열은 그 값을 다르게 치른다. 토큰 헤드는 숫자를 하나씩 디코드하고, 노이즈 제거기는 청크 전체에 정해진 횟수의 패스를 치른다. 그래서 선택은 청크 길이와 제어 예산을 따른다.
...
Skild AI는 다섯째 설계를 가장 멀리 민다. 시뮬레이션에서 로봇 몸 $100{,}000$종에 걸쳐 학습한 "옴니바디" 정책으로, 학습에서 뺀 몸을 제어하고 다리를 잃은 뒤 몇 초 안에 다시 적응한다고 보고한다([Skild AI](https://www.skild.ai/blogs/omni-bodied)). 회사의 시연이고, 같은 글이 깨끗하게 통제된 영상이 능력의 착시를 만들 수 있다고 스스로 경고한다.
