...
*Stands on [[03-deep-learning/foundations/training-at-scale|1.3 Training at Scale]], whose §5–§6 count the bytes and FLOPs a parameter costs and whose §11 finds a robot policy's decoding memory-bound on a Jetson Thor. This page asks where those FLOPs and bytes are spent: on which units of a GPU, in what order, how fast, and what a robot-learning program must do to keep them busy. It freezes its own objects, the policy network MLP-256 and the course GPU G-100 — cost objects, where D1–D6, the six tiny worked examples of [[03-deep-learning/lab-objects|0. Lab Objects]], are tensor objects. [[04-robotics/robot-systems-deployment|10. Robot Systems §3]] budgets the latency that §8 fills in, and [[06-research-practice/simulators-benchmarks-datasets|7. Simulators, Benchmarks & Datasets §2]] lists the parallel simulators that §5 explains.*

> [!note] Why this matters · 왜 배우는가
> In the physical-AI stack of [[07-research-program/index|7. Research Program §5]], this page is the compute floor under the learning-and-adaptation layer (its place is marked on the [[physical-ai-map|Physical AI Map]]): in *"Install that panel on the frame"* it performs none of the eight steps, but it decides how fast the policy that performs the fitting is trained and how long that policy takes to act. Without it a policy's cost is read off the wrong number: on the course GPU G-100, one robot's forward pass through MLP-256 takes $15.17\,\mathrm{\mu s}$, $98.9\%$ of it kernel launches rather than arithmetic, while $4{,}096$ simulated robots take $25.19\,\mathrm{\mu s}$ together, $6.15\,\mathrm{ns}$ each — which is why robot RL trains thousands of environments on one GPU, and why a faster matrix product buys the robot nothing. The same accounting returns in [[02-foundations/rl-robot-learning|7.5 RL for Robot Learning]], whose two billion simulated steps are a few GPU-hours, in [[02-foundations/tools/gpu-clusters|12.7 GPU Clusters]], which gets a training run onto a cluster's GPUs, and in [[05-construction-robotics/imitating-contact|10. Imitating Contact]], whose policy acts every $0.1\,\mathrm s$ on the robot; the page sits outside the seven blocks of the dissertation path ([[07-research-program/index|7. Research Program §8]]) and joins it when the policy trains at scale or in a parallel simulator ([[07-research-program/index|7. Research Program §9]]). After it you can price any layer's FLOPs, bytes and launches on a roofline and say whether a GPU program is bound by arithmetic, by memory or by overhead.
...
Start from the timeline, not from a kernel. A step whose timeline shows kernels separated by gaps is launch- or host-bound, and no kernel optimization will shorten it; a step whose kernels run back to back is kernel-bound, and then Nsight Compute's roofline says which roof each kernel is under. For MLP-256 the timeline would answer the question at $N=1$: three kernels of well under a microsecond between launches of 5 µs. When the gaps are the host's own Python — a loop the interpreter runs one element at a time — moving the loop into array operations is [[02-foundations/tools/python-research-code|12.3 Python for Research Code §4]]; when the GPU waits on batches the loader has not yet prepared, the pipeline, its worker processes and the bound Amdahl's law puts on them are [[02-foundations/tools/concurrency|12.8 Concurrency §7 and §9]], and staging the data on a shared cluster is [[02-foundations/tools/gpu-clusters|12.7 GPU Clusters §8]].
...
*[[03-deep-learning/foundations/training-at-scale|1.3 대규모 학습]] 위에 선다. 그 페이지의 §5–§6은 파라미터 하나가 치르는 바이트와 FLOP을 세고, §11은 로봇 정책의 디코딩이 Jetson Thor에서 메모리 한계임을 보인다. 이 페이지는 그 FLOP과 바이트가 어디서 쓰이는지 묻는다. GPU의 어느 장치에서, 어떤 순서로, 얼마나 빨리 쓰이는지, 그리고 로봇 학습 프로그램이 그 장치들을 쉬지 않게 하려면 무엇을 해야 하는지다. 대상은 이 페이지가 스스로 고정하는 정책 신경망 MLP-256과 교과용 GPU G-100이고, D1–D6([[03-deep-learning/lab-objects|0. Lab Objects]]의 작은 연습 예제 여섯)가 텐서 대상인 것과 달리 비용 대상이다. [[04-robotics/robot-systems-deployment|10. 로봇 시스템 §3]]이 세우는 지연 예산을 §8이 채우고, [[06-research-practice/simulators-benchmarks-datasets|7. 시뮬레이터·벤치마크·데이터셋 §2]]가 나열하는 병렬 시뮬레이터를 §5가 설명한다.*

> [!note] 왜 배우는가 · Why this matters
> [[07-research-program/index|7. 연구 프로그램 §5]]의 피지컬 AI 스택에서 이 페이지는 학습·적응 층 밑의 계산 바닥이다([[physical-ai-map|피지컬 AI 지도]]에 그 자리가 표시되어 있다). "*저 패널을 프레임에 설치해*"의 여덟 단계 가운데 어느 것도 직접 맡지 않지만, 끼움을 수행하는 정책을 얼마나 빨리 학습시키고 그 정책이 행동하는 데 얼마나 걸리는지를 정한다. 이것 없이는 정책의 비용을 엉뚱한 숫자로 읽는다. 교과용 GPU G-100에서 MLP-256의 로봇 한 대 순전파는 $15.17\,\mathrm{\mu s}$이고 그 $98.9\%$가 산술이 아니라 커널 launch인 반면, 시뮬레이션 로봇 $4{,}096$대는 함께 $25.19\,\mathrm{\mu s}$, 한 대에 $6.15\,\mathrm{ns}$다. 로봇 RL이 GPU 하나에서 환경 수천 개를 학습시키는 이유이고, 행렬곱을 빠르게 해도 로봇에게는 아무것도 달라지지 않는 이유다. 같은 셈이 20억 시뮬레이션 스텝이 GPU 몇 시간인 [[02-foundations/rl-robot-learning|7.5 로봇 학습을 위한 RL]], 학습을 클러스터의 GPU에 올리는 [[02-foundations/tools/gpu-clusters|12.7 GPU 클러스터]], 정책이 로봇 위에서 $0.1\,\mathrm s$마다 행동하는 [[05-construction-robotics/imitating-contact|10. 접촉 모방]]에서 돌아온다. 이 페이지는 학위논문 경로([[07-research-program/index|7. 연구 프로그램 §8]])의 일곱 블록 밖에 있고, 정책을 대규모로나 병렬 시뮬레이터에서 학습시킬 때 경로에 들어온다([[07-research-program/index|7. 연구 프로그램 §9]]). 이 페이지를 마치면 어느 층이든 FLOP, 바이트, launch를 루프라인 위에서 값을 매기고, GPU 프로그램이 산술·메모리·오버헤드 가운데 무엇에 묶였는지 말할 수 있다.
...
커널이 아니라 시간선에서 시작한다. 커널 사이에 틈이 벌어진 스텝은 launch나 호스트에 묶였고, 어떤 커널 최적화도 그것을 줄이지 못한다. 커널이 빈틈없이 이어지는 스텝은 커널에 묶였고, 그때 Nsight Compute의 루프라인이 커널마다 어느 지붕 아래인지 말해 준다. MLP-256이라면 $N=1$의 질문은 시간선이 답한다. 5 µs짜리 launch 사이에 1마이크로초에 한참 못 미치는 커널 셋이다. 틈이 호스트 자신의 Python — 인터프리터가 원소를 하나씩 도는 루프 — 이라면 그 루프를 배열 연산으로 옮기는 법이 [[02-foundations/tools/python-research-code|12.3 연구 코드를 위한 Python §4]]이고, GPU가 로더가 아직 준비하지 못한 배치를 기다린다면 그 파이프라인, 워커 프로세스, 그리고 암달의 법칙이 거기에 거는 한계는 [[02-foundations/tools/concurrency|12.8 동시성 §7, §9]]이며, 공유 클러스터에서 데이터를 옮겨 두는 법은 [[02-foundations/tools/gpu-clusters|12.7 GPU 클러스터 §8]]이다.
