...
Arguably the most-used algorithm in deep learning; virtually every model in this wiki — from [[canonical-papers/notes/attention-is-all-you-need|Transformer]] to VLAs — was trained with Adam or AdamW. Follow-ups: AMSGrad, AdamW, and recent memory-efficient variants (Adafactor, 8-bit Adam, Lion).

### Connections

- Used by: essentially every note in `canonical-papers/notes/`
...
딥러닝에서 가장 많이 쓰인 알고리즘이라 해도 과언이 아니다. [[canonical-papers/notes/attention-is-all-you-need|Transformer]]부터 VLA까지 이 위키의 거의 모든 모델이 Adam/AdamW로 학습됐다. 후속: AMSGrad, AdamW, 메모리 효율 변형들(Adafactor, 8-bit Adam, Lion).

### 연결

- 쓰이는 곳: `canonical-papers/notes/`의 사실상 모든 노트
