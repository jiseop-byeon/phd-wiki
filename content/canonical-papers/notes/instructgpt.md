...
- Previous: [[canonical-papers/notes/gpt-3|GPT-3]] · Successors: DPO, Constitutional AI
...
[[canonical-papers/notes/gpt-3|GPT-3]]는 그럴듯한 다음 토큰을 예측할 뿐, 그것이 사용자가 원하는 일을 하는 것과 같지 않다: 출력은 도움이 안 되거나, 지어냈거나, 유해할 수 있었고 프롬프팅만으로는 안정적으로 고칠 수 없었다. 목적함수(LM 우도)가 목표(도움되고 정직하고 무해한 어시스턴트)와 **어긋나 있었던** 것이다. "인간이 실제로 선호하는 것"을 손실함수로 쓸 수 없다면 어떻게 최적화할까?
...
- 이전: [[canonical-papers/notes/gpt-3|GPT-3]] · 후속: DPO, Constitutional AI
