...
- **`wiki-support:`** — 이 페이지가 *자체적으로* 제공하는 깊이. 없으면 페이지가 권장
  깊이를 지원한다는 뜻. Working 권장 페이지의 `wiki-support: Literacy`는: 이 페이지는 그 주제의 논문을
  정확히 읽어낼 수준까지만 데려다주고, Working에 이르려면 링크된 원 자료·교재·코드가 필요하며 —
  페이지가 어떤 자료인지 알려준다는 뜻이다. `study-depth`와 같은 값을 명시해도 되고,
  `study-depth`보다 *높은* 값은 페이지가 요구치를 넘겨 준다는 뜻이다 — 이 주제에
...
<svg viewBox="0 0 560 226" style="max-width:100%;height:auto" role="img" aria-label="넓은 Literacy 띠에서 좁은 Mastery 띠로 아래로 갈수록 좁아지는 네 개의 띠">
  ...
  <g fill="currentColor" text-anchor="middle">
    ...
    <text x="280" y="148" font-size="10.5">Mastery</text>
    <text x="280" y="161" font-size="9" opacity="0.8">접촉이 많은 조작</text>
...
| 깊이 | 영역 | 왜 그 깊이인가 |
|---|---|---|
| **Mastery** | 접촉이 많은 조작; 힘·임피던스·어드미턴스 제어; 파지; 매니퓰레이터 동역학과 작업공간(operational space)으로 잇는 다리; 조작에 쓰는 모방학습; **원격조작과 시연 수집** | 기여와 그 가장 가까운 의존 층 — 디펜스에서 살아남아야 하는 주장들 |
...
> 승격되는 것은 접촉이 실제로 일어나는 핵심 — 로봇이 무언가에 닿을 때 일어나는 일 — 과
