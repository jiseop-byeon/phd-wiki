...
- **The practical responses**: (a) control on *forces, not positions* — the RSL line's
  ...
  soil-agnostic; (b) **learn adaptively** — soil-adaptive RL (RA-L 2022) trains across
...
| **Imitation / pretrain–fine-tune** | task level (ExACT, ExT) | leverages operator demonstrations, multi-task | where demos came from (real operators? RL experts in sim?), fine-tuning cost |
...
**AES (Baidu, Science Robotics 2021).** The industrial-scale counterpoint: standard
(non-walking) excavators from compact to 49 t, 24 h continuous material-loading
...
**The wheel-loader cluster (Tampere/Luleå/Umeå, ICRA 2024–).** Short-loading-cycle
...
- [Stentz et al. 1999 — the 1998 CMU ancestor (CiteSeerX PDF)](https://citeseerx.ist.psu.edu/document?doi=f58f271be50a3d37cd1bccb29fa7bcff439a6086&repid=rep1&type=pdf)
...
- **실전의 대응**: (a) *위치가 아니라 힘으로* 제어 — RSL 라인의 창립 수(2017)는 굴착을
  ...
  soil-adaptive RL(RA-L 2022)은 무작위화된 토질 파라미터에 걸쳐 학습하고 토질을 식별하지
...
| **모방 / 사전학습–파인튜닝** | 과제 수준 (ExACT, ExT) | 운전자 시연 활용, 멀티태스크 | 시연의 출처(실제 운전자? 시뮬레이션 속 RL 전문가?), 파인튜닝 비용 |
...
돌담 — 데모가 아니라 납품된 조경 커미션. 자율성: 측량된 작업 공간 안의 완전 과제 자율;
인간이 과제를 정의하고 감독한다. 주목할 한계: 무겁게 계측된 단 한 대의 기계 — 플랫폼
일반성이 열린 질문이었고 ExT가 나중에 공략했다.

**AES (Baidu, Science Robotics 2021).** 산업 규모의 대응점: 소형부터 49톤까지의 표준
(비보행) 굴착기, 인간급 톤/시간으로 24시간 연속 자재 적재 배치. 방법: 학습의 신규성보다
...
**휠로더 클러스터 (Tampere/Luleå/Umeå, ICRA 2024–).** 실제 로더에서의 IL 사전학습 + RL
...
- [AES — Science Robotics 2021](https://www.science.org/doi/10.1126/scirobotics.abc3164)
- [ExT (arXiv 2509.14992)](https://arxiv.org/abs/2509.14992) · [Soil-adaptive RL (ETH OA)](https://www.research-collection.ethz.ch/server/api/core/bitstreams/95ef5691-11e8-4a86-b02d-6f0e2501de9b/content)
- [Stentz et al. 1999 — 1998년 CMU 조상 (CiteSeerX PDF)](https://citeseerx.ist.psu.edu/document?doi=f58f271be50a3d37cd1bccb29fa7bcff439a6086&repid=rep1&type=pdf)
