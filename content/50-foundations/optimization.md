...
([[canonical-papers/notes/adam|Adam]]), solving MPC, planning a trajectory, and allocating
...
- **Gradient descent** from Taylor: minimizing the first-order model within a step-size
  ...
  ([[50-foundations/linear-algebra|linear algebra]]).
...
- Stochastic gradients: unbiased but noisy estimates from minibatches; noise ~ helps escape
  saddles, demands step-size decay or adaptivity — [[canonical-papers/notes/adam|Adam]] ≈
...
**MPC as a QP, written out** ([[20-robotics/index|control track]]): linear dynamics
...
- Network training = stochastic non-convex optimization
  ([[canonical-papers/notes/adam|Adam]]; [[canonical-papers/notes/batch-norm|BatchNorm]]
  reshapes the landscape's conditioning).
- [[canonical-papers/notes/lora|LoRA]] = constraining the search to a low-rank manifold.
- [[canonical-papers/notes/instructgpt|RLHF]]'s KL penalty = a soft trust-region constraint.
- Diffusion training = minimizing a variational bound; [[canonical-papers/notes/planet|PlaNet]]'s
...
최적화는 이 위키의 공용 언어다: 네트워크 학습([[canonical-papers/notes/adam|Adam]]),
...
- 테일러에서 나오는 **경사 하강**: 1차 모델을 신뢰 반경 안에서 최소화하면
  ...
  ([[50-foundations/linear-algebra|선형대수]]).
...
- 확률적 그래디언트: 미니배치의 불편이지만 시끄러운 추정; 노이즈는 안장 탈출을 돕는 대신
  스텝 감쇠나 적응성을 요구한다 — [[canonical-papers/notes/adam|Adam]] ≈ 모멘텀 +
  좌표별 곡률 대리.
...
**MPC를 QP로 완전히 써보기** ([[20-robotics/index|제어 트랙]]): 선형 동역학
...
- 네트워크 학습 = 확률적 비볼록 최적화 ([[canonical-papers/notes/adam|Adam]];
  [[canonical-papers/notes/batch-norm|BatchNorm]]은 지형의 조건수를 다듬는다).
- [[canonical-papers/notes/lora|LoRA]] = 탐색을 저랭크 다양체로 제약.
- [[canonical-papers/notes/instructgpt|RLHF]]의 KL 페널티 = 부드러운 신뢰 영역 제약.
- 디퓨전 학습 = 변분 하한 최소화; [[canonical-papers/notes/planet|PlaNet]]의 CEM 플래닝 =
  잠재 공간의 미분 불요 최적화.
