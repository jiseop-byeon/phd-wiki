...
- **Taylor expansion** is the foundation of all of optimization:
  ...
  ([[50-foundations/optimization|optimization]]).
...
- **ReLU**: mask gradient — cheap, non-saturating; the reason it beat sigmoid
  ([[canonical-papers/notes/alexnet|AlexNet]]). Dead units = permanently zero mask.
...
- **Vanishing gradients**: products of Jacobians with norms < 1 decay exponentially with
  ...
  - [[canonical-papers/notes/lstm|LSTM]]: a self-connection of weight exactly 1.0 — the
    error carousel where the product stops shrinking.
  - [[canonical-papers/notes/batch-norm|BatchNorm]]: renormalize activations so Jacobians
    stay well-scaled (better conditioning).
  - [[canonical-papers/notes/resnet|ResNet]]: $\partial(x + F(x))/\partial x = I + \partial F/\partial x$
...
- **Stop-gradient** $\text{sg}[\cdot]$: deliberately cut the graph. Reparameterization
  ([[canonical-papers/notes/vae|VAE]]) moves sampling *outside* the differentiated path;
  EMA teachers ([[canonical-papers/notes/dino|DINO]]) and RL target networks receive no
...
- Every $E[\cdot]$ in a loss becomes a minibatch mean. Every expectation you cannot sample
  through becomes a bound ([[50-foundations/information-theory|ELBO]]) or a trick
  (reparameterization; likelihood-ratio/policy gradients — [[50-foundations/rl-basics|RL basics]]).
...
4. In the [[canonical-papers/notes/vae|VAE]], why can't you backprop through
...
- **테일러 전개**가 최적화 전체의 토대다:
  ...
  ([[50-foundations/optimization|최적화]]).
...
- **ReLU**: 마스크 그래디언트 — 싸고, 포화하지 않는다; 시그모이드를 이긴 이유다
  ([[canonical-papers/notes/alexnet|AlexNet]]). 죽은 유닛 = 영원히 0인 마스크.
...
- **그래디언트 소실**: 노름 < 1인 야코비안들의 곱은 깊이/시간에 지수적으로 붕괴.
  역사 순서의 처방:
  - [[canonical-papers/notes/lstm|LSTM]]: 가중치가 정확히 1.0인 자기 연결 — 곱이 더는
    줄지 않는 오차 회전목마.
  - [[canonical-papers/notes/batch-norm|BatchNorm]]: 활성값을 재정규화해 야코비안의
    스케일을 유지 (조건수 개선).
  - [[canonical-papers/notes/resnet|ResNet]]:
...
- **Stop-gradient** $\text{sg}[\cdot]$: 그래프를 의도적으로 자르기. reparameterization
  ([[canonical-papers/notes/vae|VAE]])은 샘플링을 미분 경로 *밖으로* 옮기고, EMA
  교사([[canonical-papers/notes/dino|DINO]])와 RL 타깃 네트워크는 설계상 그래디언트를 받지
  않는다. 논문의 점선 화살표가 이것이다.

### 6. 구현자의 눈으로 수식 읽기

- 손실의 모든 $E[\cdot]$는 미니배치 평균이 된다. 통과해서 샘플링할 수 없는 기댓값은
  하한([[50-foundations/information-theory|ELBO]])이나 트릭(reparameterization;
  우도비/정책 그래디언트 — [[50-foundations/rl-basics|RL 기초]])이 된다.
...
3. 순방향 모드 자동 미분은 왜 *입력 파라미터마다* 한 패스가 들고, 그것이 7B 모델에 왜
   치명적인가?
4. [[canonical-papers/notes/vae|VAE]]에서 $z \sim \mathcal{N}(\mu, \sigma^2)$를 직접
