...
- Shape discipline: $(m\times n)(n\times 1) = (m \times 1)$. Reading shapes is how you read
  ...
  [[canonical-papers/notes/attention-is-all-you-need|Transformer]] type-checks in one line.
- **Dot product and angle**: $\langle a,b\rangle = \|a\|\|b\|\cos\theta$. Cosine similarity
  $= \langle a,b\rangle / (\|a\|\|b\|)$ — the retrieval metric of
  [[canonical-papers/notes/clip|CLIP]].
...
- Low-rank structure recurs everywhere: [[canonical-papers/notes/lora|LoRA]] assumes weight
...
- Why you care, concretely:
  ...
  - **Optimization landscapes**: for quadratic loss $\frac12 x^\top H x$, gradient descent
    ...
    **condition number** $\kappa = \lambda_{max}/\lambda_{min}$ *is* the difficulty of the
    problem — the fact [[canonical-papers/notes/adam|Adam]] and
    [[canonical-papers/notes/batch-norm|BatchNorm]] exist to fight.
...
- **Eckart–Young**: the best rank-$k$ approximation (in $\|\cdot\|_F$ or $\|\cdot\|_2$) is
  ...
  [[canonical-papers/notes/lora|LoRA]], model compression, and PCA.
...
- Manifold hypothesis: real data occupies a low-dimensional surface inside pixel space —
  the implicit justification for latent spaces ([[canonical-papers/notes/vae|VAE]],
  [[canonical-papers/notes/latent-diffusion|latent diffusion]]).
...
3. A discrete system $x_{t+1} = Ax_t$ has eigenvalues $0.9, 1.02$. What happens, and along
   which direction?
4. Why does [[canonical-papers/notes/lora|LoRA]] initialize $B = 0$? (What map does
...
- 모양의 규율: $(m\times n)(n\times 1) = (m \times 1)$. 모양 읽기가 구조 읽기다.
  ...
  [[canonical-papers/notes/attention-is-all-you-need|Transformer]] 전체가 한 줄로 타입
  검사된다.
- **내적과 각도**: $\langle a,b\rangle = \|a\|\|b\|\cos\theta$. 코사인 유사도
  $= \langle a,b\rangle / (\|a\|\|b\|)$ — [[canonical-papers/notes/clip|CLIP]]의 검색 지표.
...
- 저랭크 구조는 도처에서 반복된다: [[canonical-papers/notes/lora|LoRA]]는 가중치
...
- 구체적으로 왜 중요한가:
  ...
  - **최적화 지형**: 이차 손실 $\frac12 x^\top H x$에서 경사 하강은 고유방향별로
    ...
    [[canonical-papers/notes/adam|Adam]]과 [[canonical-papers/notes/batch-norm|BatchNorm]]이
    존재하는 이유가 이것과의 싸움이다.
...
- **Eckart–Young**: 최적 랭크-$k$ 근사($\|\cdot\|_F$·$\|\cdot\|_2$ 기준)는 절단 SVD
  $\sum_{i\le k}\sigma_i u_i v_i^\top$이다. [[canonical-papers/notes/lora|LoRA]], 모델 압축,
  PCA의 수학적 면허장.
...
선형대수는 제어의 언어 *그 자체*다 ([[20-robotics/index|제어 트랙]]):
...
- 다양체 가설: 실제 데이터는 픽셀 공간 속 저차원 곡면 위에 산다 —
  잠재 공간([[canonical-papers/notes/vae|VAE]],
  [[canonical-papers/notes/latent-diffusion|latent diffusion]])의 암묵적 정당화.
...
3. 이산 시스템 $x_{t+1} = Ax_t$의 고유값이 $0.9, 1.02$다. 무슨 일이, 어느 방향으로
   일어나는가?
4. [[canonical-papers/notes/lora|LoRA]]는 왜 $B = 0$으로 초기화하는가? (0스텝에서
   $W_0 + BA$는 어떤 사상과 같은가?)
