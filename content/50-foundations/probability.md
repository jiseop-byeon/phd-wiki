...
- Variance $\text{Var}(X) = E[X^2] - E[X]^2$; covariance
  ...
  $\Sigma = E[(x-\mu)(x-\mu)^\top]$ is PSD ([[50-foundations/linear-algebra|linear algebra]]).
...
([[50-foundations/information-theory|information theory]]) — the "least presumptuous" choice.
...
- **MLE**: $\hat\theta = \arg\max_\theta \sum_i \log p(x_i|\theta)$.
  ...
  **MSE regression is MLE under Gaussian noise; cross-entropy is MLE for categorical
  outputs.** Every pretraining objective in [[canonical-papers/canonical-list|the paper list]]
  is an MLE (or a bound on one — [[canonical-papers/notes/vae|ELBO]]).
...
- Estimator quality: bias, variance, and the tradeoff between them — the vocabulary behind
  ...
  ([[50-foundations/rl-basics|policy gradients]]).
...
- A random process = an indexed family of RVs; characterized by mean function and
  ...
  analysis, [[50-foundations/signal-processing|signal processing]]).
  **White noise**: uncorrelated samples, flat spectrum — the default disturbance model and
  the $\epsilon$ of [[canonical-papers/notes/ddpm|diffusion]].
- **Markov property**: future ⟂ past | present. The modeling assumption of MDPs
  ([[50-foundations/rl-basics|RL]]), world models, and diffusion chains.
...
3. Using affine closure, show why $x_t = \sqrt{\bar\alpha_t}x_0 + \sqrt{1-\bar\alpha_t}\epsilon$
   ([[canonical-papers/notes/ddpm|DDPM]]) has the claimed distribution.
...
([[50-foundations/information-theory|정보이론]]) — "가장 덜 주제넘은" 선택.
...
- **MLE**: $\hat\theta = \arg\max_\theta \sum_i \log p(x_i|\theta)$.
  ...
  **MSE 회귀는 가우시안 노이즈 하의 MLE이고, 교차 엔트로피는 카테고리 출력의 MLE다.**
  [[canonical-papers/canonical-list|논문 리스트]]의 모든 사전학습 목적함수가 MLE(또는 그
  하한 — [[canonical-papers/notes/vae|ELBO]])다.
...
- 추정기의 품질: 편향, 분산, 그리고 그 사이의 트레이드오프 — RL 논문의 "불편(unbiased)
  ...
  ([[50-foundations/rl-basics|정책 그래디언트]]).
...
- 랜덤 프로세스 = 인덱스 달린 확률변수의 족; 평균 함수와 자기상관으로 특성화.
  ...
  **백색 잡음**: 무상관 샘플, 평평한 스펙트럼 — 기본 외란 모델이자
  [[canonical-papers/notes/ddpm|디퓨전]]의 $\epsilon$.
- **마르코프 성질**: 미래 ⟂ 과거 | 현재. MDP([[50-foundations/rl-basics|RL]]), 월드모델,
  디퓨전 체인의 모델링 가정.
...
3. 아핀 닫힘을 써서 $x_t = \sqrt{\bar\alpha_t}x_0 + \sqrt{1-\bar\alpha_t}\epsilon$
   ([[canonical-papers/notes/ddpm|DDPM]])이 주장된 분포를 갖는 이유를 보여라.
