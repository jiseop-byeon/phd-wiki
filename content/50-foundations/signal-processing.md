...
- A CNN layer is exactly a *learned bank* of such $h$'s in 2D (plus nonlinearity) —
  [[canonical-papers/notes/alexnet|AlexNet]] onward; "padding/stride" are the boundary and
...
- Fourier's claim: signals = sums of sinusoids. Deeper claim: **complex exponentials are
  the eigenfunctions of LTI systems** ([[50-foundations/linear-algebra|eigen-thinking]]) —
...
- **Phase lag is the price of causal smoothing**: every causal low-pass delays the signal —
  ...
  the **Kalman filter** ([[50-foundations/probability|derived here]]) is the optimal
...
- Poles of $H$ = eigenvalues of the state-space $A$
  ([[50-foundations/linear-algebra|control connection]]): stability = poles in the left
...
- CNN 층은 정확히 이런 $h$들의 *학습된 2D 묶음*(+ 비선형성)이다 —
  [[canonical-papers/notes/alexnet|AlexNet]] 이후 전부; "패딩/스트라이드"는 같은 연산의
  경계·샘플링 선택지다.
...
- 푸리에의 주장: 신호 = 사인파들의 합. 더 깊은 주장: **복소 지수함수는 LTI 시스템의
  고유함수다** ([[50-foundations/linear-algebra|고유값적 사고]]) — 사인파가 들어가면 같은
...
- **위상 지연은 인과적 평활화의 대가다**: 모든 인과적 저역통과는 신호를 늦춘다 — 과한
  ...
  **칼만 필터**([[50-foundations/probability|여기서 유도]])가 최적 시변 필터다.
...
- 라플라스 변환(연속) / **Z-변환**(이산)은 푸리에의 일반화: 합성곱 ↦ *전달함수*
  $H(s)$ 또는 $H(z)$와의 곱.
- $H$의 극점 = 상태공간 $A$의 고유값
  ([[50-foundations/linear-algebra|제어 연결]]): 안정성 = 극점이 좌반평면(연속) /
