...
The engineering math that pages 1–7 silently assume — collected here so nothing is
outsourced to "go review high school math." Each section says exactly which foundation
page uses it. If all of this reads easily, skip straight to
[[02-foundations/linear-algebra|1. Linear Algebra]].
...
1~7 페이지가 말없이 전제하는 공업수학 — "고교 수학을 복습하라"로 떠넘기지 않기 위해 여기
모았다. 각 절이 정확히 어느 기초 페이지에 쓰이는지 표시했다. 전부 술술 읽히면 바로
