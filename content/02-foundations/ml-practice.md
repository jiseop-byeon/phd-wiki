...
| **그래디언트 누적** | 여러 번의 순전파 그래디언트를 모아 한 번 optimizer step을 밟는다 | 예제별로 분해되는 손실과 같은 optimizer step 등의 조건에서 큰 배치와 대응한다. 대조 손실의 negatives, 배치 통계, 확률 연산은 항상 같지 않다. 예컨대 feature gather가 없으면 8 × 512는 microbatch당 negative 511개이지 4095개가 아니다 |
