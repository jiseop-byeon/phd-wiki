...
> 4. 지연: 칼만은 모델 예측이 위상 지연을 상쇄한다(예측+보정 구조). 모델: 프로세스/측정 노이즈 비를 명시적으로 써서 시변 최적 이득을 계산한다 — 손튜닝 저역통과에는 둘 다 없다. 반대로 저역통과가 이기는 경우는 운동 모델이 틀렸거나 아예 없을 때, 잡음이 가정한 통계에서 멀 때, 또는 모델링과 튜닝에 들일 여력이 없을 때다. 틀린 모델은 칼만 필터를 확신에 차서 틀리게 만든다.
