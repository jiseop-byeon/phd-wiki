...
- **데이터셋**: 기존 60개 데이터셋, 22종 로봇(단일 팔, 양팔, 사족), 500+ 기술,
  100만+ 에피소드를 RLDS로 통일. **이 숫자들은 릴리스 수치이지 초록의 수치가 아니다.**
  초록은 "로봇 22종, **21개 기관**의 협업, **527개 기술(160,266 태스크)**"이라 하고,
  일치하는 것은 로봇 수뿐이다. "34개 랩"과 에피소드 수는 프로젝트 페이지와 데이터셋
  릴리스에서 온 것이고 그 뒤로 늘었다. [[01-canonical-papers/how-to-read|how-to-read §4.6]]이
  DROID·BridgeData V2로 기록한 바로 그 판본 함정이니, 어느 쪽을 인용하는지 밝혀라.
- **RT-1-X / RT-2-X**: [[rt-1|RT-1]]과 [[rt-2|RT-2]] 구조를 혼합 데이터로 재학습 —
  의도적으로 *새 기법 없음*, 교차-신체 데이터의 효과만 분리해서 측정.

### 결과

- **소규모 데이터 영역에서** RT-1-X가 시험한 로봇 다섯 가운데 넷에서 각 랩의 원래 방법을 이긴다 — 평균 성공률은 원래 방법이나 그 로봇 데이터로만 학습한 RT-1보다 50% 높다. 몸을 건너는 양의 전이는 실재하되 한정적이다: 논문은 대규모 데이터 설정에서 RT-1-X가 해당 신체 전용 데이터만으로 학습한 RT-1 베이스라인을 능가하지 **못한다**고 밝힌다.
