...
> [[02-foundations/information-theory|5. Information Theory §2]] for the training objective and for **perplexity** ($2^{H(p,q)}$ in bits, $e^{H(p,q)}$ in nats) — the only number this paper reports that needs a definition. In-context learning is an empirical finding, not a mechanism the math predicts.
> 학습 목적함수와 **perplexity**($H$가 비트면 $2^{H(p,q)}$, nat이면 $e^{H(p,q)}$)는 [[02-foundations/information-theory|5. 정보이론 §2]] — 이 논문이 보고하는 숫자 중 정의가 필요한 것은 그것뿐이다. in-context learning은 수학이 예측한 메커니즘이 아니라 경험적 발견이다.
