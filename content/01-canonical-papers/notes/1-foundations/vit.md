...
- Purely supervised pretraining — the self-supervised chapter came later ([[01-canonical-papers/notes/1-foundations/mae|MAE]], DINO).
...
- Next: [[01-canonical-papers/notes/1-foundations/mae|MAE]], CLIP
...
CNN이 비전을 지배한 것은 그 귀납 편향(지역성, 평행이동 등변성)이 이미지에 잘 맞았기 때문이다. [[01-canonical-papers/notes/1-foundations/attention-is-all-you-need|Transformer]] 이후의 시도들은 어텐션을 CNN *안에* 끼워 넣는 절충이었다. ViT는 직설적으로 물었다: 합성곱이 필요하긴 한가? 데이터가 충분하면 귀납 편향을 대체하지 않을까?
...
- 순수 지도 사전학습 — 자기지도 챕터는 이후에 온다([[01-canonical-papers/notes/1-foundations/mae|MAE]], DINO).
...
- 이전: [[01-canonical-papers/notes/1-foundations/resnet|ResNet]] (왕좌에서 내려온 베이스라인), [[01-canonical-papers/notes/1-foundations/attention-is-all-you-need|Transformer]]
- 다음: [[01-canonical-papers/notes/1-foundations/mae|MAE]], CLIP
