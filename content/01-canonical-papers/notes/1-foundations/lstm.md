...
- **Input gate** controls what enters the cell; **output gate** controls what the rest of the network reads. (The now-standard **forget gate** was added by Gers et al., 1999 — because on a continuous, never-reset input stream the cell state grows without bound and saturates the squashing function. The forget gate lets the cell release its own memory, which is what allows an LSTM to run indefinitely.)
...
- **입력 게이트**가 셀에 들어갈 정보를, **출력 게이트**가 네트워크가 읽어갈 정보를 통제. (지금은 표준인 **forget 게이트**는 Gers et al. 1999 — 리셋 없이 계속 흐르는 입력에서는 셀 상태가 무한히 자라 스쿼싱 함수를 포화시키기 때문에 추가됐다. forget 게이트는 셀이 자기 기억을 스스로 놓게 해 주며, LSTM이 끝없이 도는 스트림 위에서도 계속 돌 수 있는 것은 그 덕분이다.)
