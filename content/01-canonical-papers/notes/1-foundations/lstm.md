...
- **입력 게이트**가 셀에 들어갈 정보를, **출력 게이트**가 네트워크가 읽어갈 정보를 통제. (지금은 표준인 **forget 게이트**는 Gers et al. 1999 — 리셋 없이 계속 흐르는 입력에서는 셀 상태가 무한히 자라 스쿼싱 함수를 포화시키기 때문에 추가됐다. forget 게이트는 셀이 자기 기억을 스스로 놓게 해 주고, LSTM이 무한히 돌 수 있는 유일한 길이 그것이다에서 추가됨.)
