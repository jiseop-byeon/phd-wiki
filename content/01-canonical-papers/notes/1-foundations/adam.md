...
> Read [[02-foundations/optimization|4. Optimization §3]] first: it derives gradient descent from Taylor and names the condition number — not the learning rate — as the real obstacle, with the numbers in [[02-foundations/linear-algebra|1. Linear Algebra §3]], and names Adam as "momentum + a per-coordinate curvature proxy". The paper is what it takes to make that sentence work in floating point (bias correction, $\epsilon$).
