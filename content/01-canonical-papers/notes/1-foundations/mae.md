...
> Read [[01-canonical-papers/notes/1-foundations/vit|ViT]] first (this is BERT-style pretraining *for* ViT), plus [[02-foundations/neural-network-basics|0.8 §6]] for token/patch/embedding vocabulary. The loss is plain MSE on pixels — [[02-foundations/probability|3. Probability §4]] explains why that is Gaussian MLE.
> [[01-canonical-papers/notes/1-foundations/vit|ViT]]를 먼저 읽어라(이 논문은 ViT를 *위한* BERT식 사전학습이다). 토큰·패치·임베딩 어휘는 [[02-foundations/neural-network-basics|0.8 §6]]. 손실은 픽셀에 대한 평범한 MSE이고, 그것이 왜 가우시안 MLE인지는 [[02-foundations/probability|3. 확률 §4]].
...
- 손실: 가린 패치에 대해서만 단순 MSE. 음의 쌍(negative pair, 불일치 쌍)도, 모멘텀 인코더도, 크롭 이상의 증강도 없음 — 대조학습 파이프라인보다 급진적으로 단순하다.
...
- [ ] Explain why images need 75% masking where language does not (redundancy) · 이미지에서 75% 마스킹이 필요한 이유(언어 대비 중복성)를 설명할 수 있다
- [ ] Say how the asymmetric encoder/decoder buys computational efficiency · 비대칭 인코더/디코더가 계산 효율을 어떻게 사는지 말할 수 있다
- [ ] State the limit of a pixel-reconstruction objective and the move to latent prediction (I-JEPA) · 픽셀 복원 목표의 한계와 잠재 예측(I-JEPA)으로의 이동을 말할 수 있다
- [ ] Say what "strong under fine-tuning, weak under linear probing" implies for choosing a backbone · 파인튜닝엔 강하고 linear probe엔 약한 특성이 백본 선택에 주는 함의를 말할 수 있다
