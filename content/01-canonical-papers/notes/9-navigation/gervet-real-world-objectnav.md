...
**건설의 경우**: 논증 자체는 일반화된다. 현장 시뮬레이션은 논문이 지목한 두 축 모두에서 주택 시뮬레이션보다 나쁘다. 이미지가 현실에서 더 멀고, 실패 양상이 덜 모델링되어 있다. 따라서 현장 시뮬레이터에서만 검증된 정책은 반증이 나오기 전까지 23% 영역에 있다고 가정해야 한다. 실배치 성능은 미확인으로 두고 같은 프로토콜의 실제 현장 시험을 요구한다.
