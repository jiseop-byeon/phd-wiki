...
> [[01-canonical-papers/notes/3-vlm/clip|CLIP]] (the frozen vision side) and [[02-foundations/neural-network-basics|0.8 §6]] for *frozen* — the whole design question here is which parts receive gradient and which do not, so that one word carries the paper.
> [[01-canonical-papers/notes/3-vlm/clip|CLIP]](얼어붙은 비전 쪽)과 *frozen*의 뜻은 [[02-foundations/neural-network-basics|0.8 §6]] — 여기서 설계 질문 전체가 "어느 부분이 그래디언트를 받고 어느 부분이 받지 않는가"이므로, 그 한 단어가 논문을 떠받친다.
