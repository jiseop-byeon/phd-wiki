...
- Contrastive objective is markedly more compute-efficient than generative alternatives — ~4× vs a bag-of-words prediction baseline. Figure 2 prints only that 4× and a 3× from transformer captioning to bag-of-words; the ~12× against captioning is those two multiplied, not a number the paper states.
...
- 대조 목적함수는 생성적 대안보다 눈에 띄게 연산 효율적 — 논문의 비교에서 bag-of-words 예측 대비 약 4배. 그림 2가 싣는 것은 그 4배와, 트랜스포머 캡셔닝에서 bag-of-words로 가는 3배뿐이다. 캡셔닝 대비 약 12배는 그 둘을 곱한 값이지 논문이 적은 수가 아니다.
