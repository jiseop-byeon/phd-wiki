...
> [[04-robotics/geometric-perception-calibration|3.5 Geometric Perception §3]] for what a point cloud is and which frame it lives in. The paper's core is one property, and it is not a linear-algebra one: max-pooling over points is **permutation-invariant**, which is exactly what an unordered set demands. Max-pooling is not a linear map, so do not go looking for this in the linear-algebra page.
