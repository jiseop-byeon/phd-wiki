...
> [[02-foundations/rl-robot-learning|7.5 RL for Robot Learning §1 and §4]] — the recipe is imitation pretraining followed by RL fine-tuning on hardware, which is exactly the RLFT pattern §9 describes, including its safety-envelope question.
> [[02-foundations/rl-robot-learning|7.5 로봇 학습을 위한 RL §1·§4]] — 레시피가 모방 사전학습 후 실기계 RL 파인튜닝이고, 이는 §9가 기술하는 RLFT 패턴 그대로다(안전 영역 문제까지 포함해서).
