...
**Why read it**: it is the middle step of the Michigan line. [[vision-guided-assembly|Feng 2015]] showed marker-based vision guidance; Lundeen replaces "detect a fiducial" with "sense the actual as-built geometry and re-plan," so the robot handles per-instance variability autonomously instead of replaying fixed trajectories; [[liang-lfd|Liang 2020]] then replaces the remaining task scripting with human demonstration. Evidence is a demonstration of sensor-driven adaptive execution with autonomy at the task-execution level inside a structured experiment. It is a **testbed, not a site**. The paper's premise describes a robot on a construction site with a BIM, but the experiments used two KUKA KR120 arms, one carrying a custom caulk dispenser and one an LMI Gocator 2350 2D laser profiler, with each specimen mounted on a frame near the joint's as-designed location (§4, Fig. 14). The specimens were square-edged boards and a door jamb and casing. Across five experiments the robot located the joint's centre with a mean norm position error of 0.11 mm and orientation error of 1.1°, and filled the joints with caulk that still showed residue and, in one case, excess in concave regions (§6). Registration uses Generalized Resolution Correlative Scan Matching against the BIM geometry.
...
**한 줄 요약**: 건설 매니퓰레이터가 실제 시공(as-built) 기하를 감지해 모션 계획과 과제 실행을 즉석에서 적응시킨다 — 건설이 공장 자동화와 다른 핵심, 즉 "작업 대상은 도면과 결코 일치하지 않는다"를 실제로 구현한 논문.
...
**읽는 이유**: 미시간 계열의 중간 단계다. [[vision-guided-assembly|Feng 2015]]가 마커 기반 비전 유도를 보였다면, Lundeen은 "마커를 검출한다"를 "실제 시공 기하를 감지하고 다시 계획한다"로 바꿔, 로봇이 고정 궤적을 재생하는 대신 개체별 변동을 자율적으로 처리하게 한다; 이어서 [[liang-lfd|Liang 2020]]이 남은 과제 스크립팅을 인간 시연으로 대체한다. 증거는 센서 기반 적응 실행의 시연이고, 자율성은 구조화된 실험 안의 과제 실행 수준이다. **현장이 아니라 테스트베드**다. 논문의 전제는 BIM을 가진 로봇이 건설 현장에 있는 상황이지만, 실험은 KUKA KR120 팔 두 대로 했다. 한 팔에는 자체 제작한 코킹 디스펜서를, 다른 팔에는 LMI Gocator 2350 2D 레이저 프로파일러를 달았고, 시편은 하나씩 설계상 조인트 위치 근처의 프레임에 고정했다(§4, 그림 14). 시편은 모서리가 각진 판재와 문틀·문선이었다. 다섯 번의 실험에서 로봇은 조인트 중심을 평균 노름 위치 오차 0.11 mm, 방향 오차 1.1°로 찾았고, 코킹으로 조인트를 채웠지만 양옆에 잔여물이 남았고 한 실험에서는 오목한 부분에 코킹이 넘쳤다(§6). 정합은 BIM 형상에 대한 Generalized Resolution Correlative Scan Matching으로 한다.
