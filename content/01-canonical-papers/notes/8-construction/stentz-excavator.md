...
**방법**: 두 대의 스캐닝 레이저 거리계가 작업 공간을 감지한다 — 하나는 적재할 트럭을 정위치화하고, 다른 하나는 토사면을 매핑한다. 실행(executive) 계층이 토사면의 굴착점과 트럭 적재함 위의 투하점을 선택하고, 모션 계획·제어가 굴착-선회-투하 사이클을 실행하며, 장애물 탐지는 무언가 작업 공간에 들어오면 기계를 정지시킨다. 기여는 학습 정책이나 단일 신규 알고리즘이 아니라 *통합 자율성* — 실제 하드웨어에서 전체 사이클을 닫은 것 — 이다.
