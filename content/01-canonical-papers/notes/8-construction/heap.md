...
- **Sensing**: dual-antenna GNSS-RTK for global pose, IMU, joint encoders/cylinder
  pressures, and cab-mounted LiDAR building local terrain maps
...
embankment/grading on real outdoor sites; slope operation via active chassis adaptation;
tasks specified from geometric files and executed **fully autonomously inside a surveyed,
geofenced workspace with staff supervising** — no public traffic, human reset on faults.
...
**Limitations & open gaps the paper leaves** (and follow-ups attacked): one
heavily-instrumented, one-of-a-kind machine — *platform generality* is untested (ExT's
motivation); no learning in the loop (Egli's RL line); productivity was not benchmarked
...
- **구동 개조**: 순정 유압 파일럿단을 전기 구동 파일럿 밸브로 교체; **다리 실린더의 서보
  ...
  가능하게 한 하드웨어 사실이다.
- **센싱**: 전역 pose용 이중 안테나 GNSS-RTK, IMU, 관절 엔코더/실린더 압력, 그리고 국소
  지형 지도를 만드는 캡 장착 LiDAR (후속 연구에서
...
제방/정지 작업; 능동 섀시 적응을 통한 경사 작업; 기하 파일로 지정된 과제를 **측량·지오펜스
된 작업 공간 안에서 직원 감독 하에 완전 자율로** 실행 — 공공 통행 없음, 결함 시 인간
리셋. 같은 플랫폼의 후속: [6m 돌담](https://ethz.ch/en/news-and-events/eth-news/news/2023/11/autonomous-excavator-constructs-a-six-metre-high-dry-stone-wall.html)
...
**논문이 남긴 한계와 공백** (후속이 공략한 것): 무겁게 계측된 단 한 대의 기계 — *플랫폼
...
> 이 논문의 주장은 "자율 보행 굴착기 시스템의 실증"이다 — 새로운 학습 방법론의 기여가 아니다. 시스템/플랫폼 논문의 기준(통합 완성도, 재사용성, 후속 연구를 가능하게 했는가)으로 평가해야 공정하고, 그 기준으로는 이 분야 최고 수준이다. "autonomous"의 운용적 의미도 정확히: 측량된 작업 공간, 직원 감독, 인간 리셋 — 무감독 배치가 아니다.

### 읽고 나면 말할 수 있어야 하는 것 · After reading (★)

- [ ] 구동 개조(전기 파일럿 밸브, 압력 센싱)가 왜 힘 기반 굴착의 전제인지 말할 수 있다
- [ ] 센싱 스택(GNSS-RTK/IMU/LiDAR/압력)과 소프트웨어 구조(매핑→계획→힘 제어)를 단계별로 말할 수 있다
- [ ] 평가의 성격 — 측량된 현장, 감독 하 완전 과제 자율, 인간 리셋 — 을 정확히 기술할 수 있다
