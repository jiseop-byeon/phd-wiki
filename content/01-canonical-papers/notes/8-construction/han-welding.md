...
> Vision locates the joint so welding motion can follow the encountered workpiece rather than only a nominal path. Automated and interactive modes place different decisions with the robot and worker; detecting a seam does not eliminate the need to define that authority.
...
> Autonomous welding refers to the detect-and-weld operation under the demonstrated preparation and mode. It does not include every navigation, setup, inspection, or recovery step. Identify what the worker does in each operating mode before comparing autonomy.
...
> 시각이 이음부를 찾으면 명목 경로만 따르지 않고 마주친 작업물에 맞춰 용접할 수 있다. 자동·상호작용 모드는 로봇과 작업자에게 다른 결정을 맡긴다. 이음 검출이 권한 정의까지 대신하지는 않는다.
...
> 자율 용접은 시연한 준비·모드 아래의 검출–용접 동작이다. 모든 이동, 설치, 검사, 회복을 포함하지 않는다. 자율성을 비교하기 전에 모드별 작업자의 역할을 확인한다.
