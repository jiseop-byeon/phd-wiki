...
>
> where $\Gamma_{ijk}$ are the Christoffel symbols of [[02-foundations/manipulator-kinematics-dynamics|10. §4]]; the form is quadratic because the chain rule turns $\dot M$ into $\sum_k (\partial M/\partial\theta_k)\,\dot\theta_k$, one more velocity beside the one already in $M\dot\theta$.
>
...
> $$c_i(\theta, \dot\theta) = \sum_{j,k}\Gamma_{ijk}(\theta)\,\dot\theta_j\dot\theta_k, \qquad \Gamma_{ijk} = \tfrac12\Bigl(\frac{\partial M_{ij}}{\partial\theta_k} + \frac{\partial M_{ik}}{\partial\theta_j} - \frac{\partial M_{jk}}{\partial\theta_i}\Bigr)$$
>
> 여기서 $\Gamma_{ijk}$는 [[02-foundations/manipulator-kinematics-dynamics|10. §4]]의 크리스토펠 기호다. 이차식이 되는 것은 연쇄법칙이 $\dot M$을 $\sum_k (\partial M/\partial\theta_k)\,\dot\theta_k$로 바꾸어, $M\dot\theta$에 이미 있던 속도 옆에 속도 하나를 더 세우기 때문이다.
>
