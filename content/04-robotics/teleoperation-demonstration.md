...
- **State-action consistency.** If the operator reacts to something the robot's sensors did
  ...
  will teach the policy that behaviour. On S1, the construction track's facade-panel task ([[05-construction-robotics/site-engineering|2.5]]), the loss is measurable: a camera that misses the
...
- **상태-행동 일관성.** 조작자가 로봇의 센서가 기록하지 않은 무언가에 반응했다면 — 소리,
  ...
  없는 행동을 담게 되고, 아무리 많아도 그 행동을 정책에 가르치지 못한다. S1(건설 트랙의 외장 패널 과제, [[05-construction-robotics/site-engineering|2.5]])에서는 이 손실을 잴 수 있다.
