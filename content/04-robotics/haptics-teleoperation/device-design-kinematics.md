...
> Plant **P3** from [[02-foundations/lab-plants|0.6 Lab Plants]]. The Jacobian $v=J(q)\dot q$ and the statics relation $\tau=J^\top F$ from [[04-robotics/modern-robotics/ch05-velocity-kinematics|MR ch.5]], and the idea of impedance versus admittance control from [[04-robotics/force-compliance-control|13. Force & Compliance Control §2]], and the human limits a device is designed against from [[04-robotics/haptics-teleoperation/human-haptics-psychophysics|24.1 Human Haptics & Psychophysics]]; this page re-derives $\tau=J^\top F$ but moves quickly.
> [[02-foundations/lab-plants|0.6]]의 장치 **P3**. [[04-robotics/modern-robotics/ch05-velocity-kinematics|MR 5장]]의 야코비안 $v=J(q)\dot q$와 정역학 관계 $\tau=J^\top F$, 그리고 [[04-robotics/force-compliance-control|13. 힘·컴플라이언스 제어 §2]]의 임피던스 대 어드미턴스 제어 개념, 그리고 장치 설계의 기준이 되는 사람의 한계는 [[04-robotics/haptics-teleoperation/human-haptics-psychophysics|24.1 인간 햅틱과 정신물리학]]. 이 페이지도 $\tau=J^\top F$를 다시 유도하지만 빠르게 지나간다.
...
Worked example, on a generic two-joint device because P3's own Jacobian is the scalar $r_m$ of the Worked case: for $J=\begin{bmatrix}0.2&0.1\\0&0.15\end{bmatrix}$ m/rad and $F=[5,-2]^\top$ N,
...
From the person come five requirements, each for a reason. The **workspace** must cover the motions the task asks of the hand, or the operator has to clutch and re-grasp in the middle of it. The **grasp** — stylus, thimble or handle — decides where on the hand the force lands, and so which receptors read it. **Comfortable continuous and peak force** are two numbers, because fingers explore at $5$–$15\,\mathrm{N}$ and press $50$–$100\,\mathrm{N}$ at most ([[04-robotics/haptics-teleoperation/human-haptics-psychophysics|24.1 §1]]): the device must match the first all session and survive the second. **Perceptual bandwidth** runs far above motion bandwidth — the hand feels vibration to about $1\,\mathrm{kHz}$ and commands motion at a few hertz — so the device must stay quiet and accurate far above any speed it is driven at. And **safety** bounds the rest, since a device strong enough to render a wall can hurt a hand when its controller fails. From the virtual task come six. The **minimum free-space impedance** and the **maximum stable wall stiffness** are the two ends of Z-width ([[04-robotics/haptics-teleoperation/rendering-sampling-stability|24.4 §4]]). The **directions of force** fix how many axes must be actuated; P3 has one. The **update rate**, with the device damping, sets the stiffness ceiling. The **collision complexity** decides whether contact can be found inside one period at all, which is why complex scenes move geometry to a slower loop ([[04-robotics/haptics-teleoperation/rendering-in-practice|24.9 §1]]). And the **desired cue** — force, vibration or texture — sets the actuator bandwidth. A useful design maximizes the intersection; no scalar “best haptic device” captures it.
...
**The two ends side by side, for stiffness.** Write the task's number beside the person's on P3 and the collision is arithmetic. A wall judged rigid by touch alone, $25\,\mathrm{N/mm}$, is $15.6$ times P3's sampled-data ceiling $2b/T = 1600\,\mathrm{N/m}$ ([[04-robotics/haptics-teleoperation/rendering-sampling-stability|24.4 §2]]), and that ceiling names the only two knobs that could close the gap. A faster loop would need $T \le 2b/K = 64\,\mathrm{\mu s}$, a $15.6\,\mathrm{kHz}$ servo. More device damping would need $b \ge KT/2 = 12.5\,\mathrm{N{\cdot}s/m}$, and that is where the two ends meet head-on: at $5\,\mathrm{cm/s}$ that damper alone drags with $0.625\,\mathrm{N}$, and with the inertial term the free-space floor becomes $0.04 \times 5 + 12.5 \times 0.05 = 0.825\,\mathrm{N}$, about twenty times the $0.04\,\mathrm{N}$ a person controls. Even with both knobs turned, the encoder of §4 would not render the wall: at $25\,\mathrm{N/mm}$ one count is worth $1.53\,\mathrm{N}$, so the amplifier's whole $2.0\,\mathrm{N}$ is spent in $0.08\,\mathrm{mm}$, $1.3$ counts, and the wall would be felt as one step rather than a spring. At the ceiling itself the same $2.0\,\mathrm{N}$ spreads over $1.25\,\mathrm{mm}$ and $20$ counts. So the honest design statement is that P3 renders a firm wall, not a rigid one, and a redesign chooses which of rate, damping and resolution to buy first; the problem set's halved pulley is one such purchase, paid for in felt mass.
...
예제. P3 자신의 야코비안은 계산 절의 스칼라 $r_m$이므로, 여기서는 일반적인 2관절 장치를 쓴다. $J=\begin{bmatrix}0.2&0.1\\0&0.15\end{bmatrix}$ m/rad, $F=[5,-2]^\top$ N일 때
...
전류가 직접적인 토크 변수다. PWM duty는 평균 단자 전압을 근사적으로 제어할 뿐이고, 전류는 여전히 저항·인덕턴스·역기전력·스위칭·부하에 달려 있다(H-브리지, duty, 그리고 권선의 $L/R$이 남기는 리플은 [[02-foundations/basic-circuits-electronics|0.6.2 기초 회로와 전자 §6]]). Stall torque는 짧은 시간의 동작점이지 연속 힘 정격이 아니다. 열 한계·포화·torque ripple(전류가 일정해도 회전자 각도에 따라 출렁이는 토크)·증폭기의 전류/전압 한계를 힘 범위 안에 포함해야 한다. 같은 두 방정식을 토크–속도 선, 전류·전압 한계와 함께 팔 관절 위에서 푼 것이 [[04-robotics/actuators-drives|10.5 액추에이터·구동계 §1–§3]]이다.

전달장치는 토크를 대략 $n$배, 반사 관성을 대략 $n^2$배로 만든다. [[02-foundations/basic-mechanics|0.6.1 기초 역학 §9]]가 이 페이지의 캡스턴 위에서 둘을 에너지로부터 유도한다. 기어는 backlash와 마찰을 더할 수 있고, capstan(작은 모터 풀리와 큰 출력 드럼에 케이블을 감은 전달장치)은 backlash가 작지만 장력·정렬·미끄럼 없는 접촉을 요구한다. 큰 감속비는 장치를 강하지만 무겁게 느껴지게 만들며, 이는 자유공간 투명성에 나쁘다. $n$과 $n^2$의 이 비대칭이 햅틱 메커니즘 설계에서 결과가 가장 큰 사실 하나이므로 양쪽 모두에 정의를 준다. 비를 $n$으로 적는 것은 엔코더의 $N=1024$ counts/rev와 구별하기 위해서다. 둘은 전혀 다른 양인데 대부분의 교과서가 같은 글자를 쓴다.
...
사람 쪽에서 요구가 다섯 나오고, 저마다 이유가 있다. **작업공간**은 과제가 손에 요구하는 움직임을 덮어야 한다. 그러지 못하면 조작자가 과제 도중에 클러치로 장치를 떼었다가 다시 잡아야 한다. **파지** — 스타일러스, 골무, 핸들 — 는 힘이 손의 어디에 닿는지, 따라서 어느 수용기가 그것을 읽는지를 정한다. **편안한 지속 힘과 최대 힘**은 두 숫자다. 손가락은 $5$–$15\,\mathrm{N}$으로 탐색하고 많아야 $50$–$100\,\mathrm{N}$으로 누르므로([[04-robotics/haptics-teleoperation/human-haptics-psychophysics|24.1 §1]]), 장치는 앞의 것을 세션 내내 맞추고 뒤의 것을 견뎌야 한다. **지각 대역폭**은 운동 대역폭보다 훨씬 위에 있다 — 손은 진동을 약 $1\,\mathrm{kHz}$까지 느끼지만 운동은 몇 헤르츠로 명령한다. 그래서 장치는 사용자가 몰아가는 어떤 속도보다 훨씬 위에서까지 조용하고 정확해야 한다. 그리고 **안전**이 나머지 전부를 묶는다. 벽을 렌더링할 만큼 센 장치는 제어기가 고장 나면 손을 다치게 할 수 있기 때문이다. 가상 과제 쪽에서는 여섯이 나온다. **자유공간 최소 임피던스**와 **안정하게 낼 수 있는 최대 벽 강성**은 Z-width의 두 끝이다([[04-robotics/haptics-teleoperation/rendering-sampling-stability|24.4 §4]]). **힘의 방향**은 몇 개의 축을 구동해야 하는지 정한다. P3는 하나다. **갱신 주기**는 장치 댐핑과 함께 강성 천장을 정한다. **충돌 복잡도**는 한 주기 안에 접촉을 찾을 수 있는지부터 정하고, 그래서 복잡한 장면은 기하를 더 느린 루프로 옮긴다([[04-robotics/haptics-teleoperation/rendering-in-practice|24.9 §1]]). 그리고 **원하는 cue** — 힘, 진동, 질감 — 가 액추에이터 대역폭을 정한다. 좋은 설계는 이 두 집합의 교집합을 최대로 만든다. "최고의 햅틱 장치"라는 하나의 스칼라 지표는 존재하지 않는다.
...
**강성으로 본 양쪽 끝, 나란히.** P3에서 과제의 숫자를 사람의 숫자 옆에 적으면 충돌이 산수가 된다. 촉각만으로 단단하다고 판단되는 벽 $25\,\mathrm{N/mm}$는 P3의 샘플링 천장 $2b/T = 1600\,\mathrm{N/m}$([[04-robotics/haptics-teleoperation/rendering-sampling-stability|24.4 §2]])의 $15.6$배이고, 그 천장이 틈을 메울 수 있는 손잡이 둘을 그대로 가리킨다. 루프를 빠르게 하려면 $T \le 2b/K = 64\,\mathrm{\mu s}$, 곧 $15.6\,\mathrm{kHz}$ 서보가 필요하다. 장치 댐핑을 늘리려면 $b \ge KT/2 = 12.5\,\mathrm{N{\cdot}s/m}$가 필요한데, 바로 여기서 양쪽 끝이 정면으로 부딪힌다. $5\,\mathrm{cm/s}$에서 그 댐퍼 하나가 $0.625\,\mathrm{N}$으로 끌고, 관성 항까지 더하면 자유공간 바닥이 $0.04 \times 5 + 12.5 \times 0.05 = 0.825\,\mathrm{N}$, 사람이 조절하는 $0.04\,\mathrm{N}$의 약 스무 배가 된다. 두 손잡이를 다 돌려도 §4의 엔코더가 그 벽을 렌더링하지 못한다. $25\,\mathrm{N/mm}$에서 한 카운트는 $1.53\,\mathrm{N}$이라 증폭기의 $2.0\,\mathrm{N}$ 전부가 $0.08\,\mathrm{mm}$, $1.3$카운트 안에서 다 쓰이고, 벽은 스프링이 아니라 계단 하나로 느껴진다. 천장 자체인 $1600\,\mathrm{N/m}$에서는 같은 $2.0\,\mathrm{N}$이 $1.25\,\mathrm{mm}$, $20$카운트에 퍼진다. 그러니 정직한 설계 진술은 P3가 강체 벽이 아니라 단단한 벽을 렌더링한다는 것이고, 재설계는 주기·댐핑·분해능 가운데 무엇을 먼저 살지 고른다. 과제의 반으로 줄인 풀리가 그런 구매 하나이고, 값은 느껴지는 질량으로 치른다.
