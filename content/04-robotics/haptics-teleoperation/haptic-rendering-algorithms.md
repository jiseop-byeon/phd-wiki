...
**In depth rather than stiffness.** At the amplifier's $2.0\,\mathrm{N}$ the catalog wall lets the handle sink $2.0/400 = 5.0\,\mathrm{mm}$, where a $25\,\mathrm{N/mm}$ wall would stop it in $0.08\,\mathrm{mm}$, $62.5$ times shallower; even P3's ceiling wall is $25000/1600 = 15.6$ times too soft. The softness a user feels is that extra depth, and the catalog wall's full-force sink is deeper than the Worked case's whole $4\,\mathrm{mm}$ plate.

**One pass of the loop, on the running object.** Put the handle $1\,\mathrm{mm}$ into the plate, at $x = 0.0310\,\mathrm{m}$. *Read*: the encoder reports whole counts of $\Delta x = 2\pi r_m/N = 6.1359\times10^{-5}\,\mathrm{m}$ ([[04-robotics/haptics-teleoperation/device-design-kinematics|24.3 §4]]); $0.0310\,\mathrm{m}$ is $505.2$ of them, so the loop sees $\hat x = 505\,\Delta x = 0.030986\,\mathrm{m}$, $14\,\mathrm{\mu m}$ short. *Decide*: $\hat x$ lies between the faces at $0.030$ and $0.034$, nearer the entry face, so the point is in contact. *Compute*: $F = -K(\hat x - x_w) = -400 \times 0.000986 = -0.394\,\mathrm{N}$, against $-0.400\,\mathrm{N}$ at the true position. *Send*: $\tau_m = r_mF = 0.010 \times (-0.394) = -3.94\,\mathrm{mN{\cdot}m}$ to the motor. On one axis and one plate all four fit easily in a $1\,\mathrm{ms}$ period. The decide step is the one that grows with the scene, which is why [[04-robotics/haptics-teleoperation/rendering-in-practice|24.9 §1]] moves geometry to a slower loop, and it is also where §2 and §3 part ways: *nearer the entry face* is a judgement the penalty law remakes from scratch every period, and the proxy remembers.
...
**강성이 아니라 깊이로.** 증폭기의 $2.0\,\mathrm{N}$에서 카탈로그 벽은 핸들을 $2.0/400 = 5.0\,\mathrm{mm}$ 가라앉게 하는데, $25\,\mathrm{N/mm}$ 벽이라면 $0.08\,\mathrm{mm}$에서, $62.5$배 얕게 멈춘다. P3의 천장 벽조차 $25000/1600 = 15.6$배 무르다. 사용자가 느끼는 무름이 바로 그 여분의 깊이이고, 카탈로그 벽이 최대 힘에서 가라앉는 깊이는 계산 절의 $4\,\mathrm{mm}$ 판 전체보다 깊다.

**대상 위에서 루프 한 바퀴.** 핸들을 판 안으로 $1\,\mathrm{mm}$, $x = 0.0310\,\mathrm{m}$에 둔다. *읽기*: 엔코더는 $\Delta x = 2\pi r_m/N = 6.1359\times10^{-5}\,\mathrm{m}$ 단위의 정수 카운트만 보고한다([[04-robotics/haptics-teleoperation/device-design-kinematics|24.3 §4]]). $0.0310\,\mathrm{m}$는 그 $505.2$개이므로 루프는 $\hat x = 505\,\Delta x = 0.030986\,\mathrm{m}$, 참값보다 $14\,\mathrm{\mu m}$ 모자란 위치를 본다. *판단*: $\hat x$가 $0.030$과 $0.034$의 두 면 사이, 진입면 쪽에 있으므로 점은 접촉 중이다. *계산*: $F = -K(\hat x - x_w) = -400 \times 0.000986 = -0.394\,\mathrm{N}$이고, 참 위치라면 $-0.400\,\mathrm{N}$이다. *보내기*: 모터에 $\tau_m = r_mF = 0.010 \times (-0.394) = -3.94\,\mathrm{mN{\cdot}m}$를 명령한다. 축 하나, 판 하나라면 넷이 $1\,\mathrm{ms}$ 주기 안에 넉넉히 들어간다. 장면과 함께 커지는 것은 판단 단계이고, 그래서 [[04-robotics/haptics-teleoperation/rendering-in-practice|24.9 §1]]이 기하를 더 느린 루프로 옮긴다. 판단 단계는 §2와 §3이 갈라지는 곳이기도 하다. *진입면에 더 가깝다*는 판단을 벌점 법칙은 매 주기 처음부터 다시 내리고, proxy는 기억한다.
