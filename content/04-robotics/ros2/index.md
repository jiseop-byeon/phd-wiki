...
3. **Interpret.** After launch, the encoder ticks up, `/tf` is silent and the motor effort is identically zero. `ros2 topic hz` on the vision topic reports $50\,\mathrm{Hz}$, yet the controller's pose callback never runs. Ordered checks — name the first three you run, and which page of this track each one lives on. One of them is a QoS mismatch: `vision` publishes with the sensor-data profile (best effort) and `controller` subscribes with the default profile (reliable). Say why that presents as “nothing happens” rather than a crash.
...
> 3. (i) `ros2 topic list` / `hz` on the vision topic ([[04-robotics/ros2/nodes-topics-messages|25.2]]): $50\,\mathrm{Hz}$, so `vision` is alive and the break sits between the topic and `controller`. `hz` subscribes best effort, so it sees a best-effort publisher that a reliable subscriber cannot. (ii) `ros2 topic info -v` on the vision topic ([[04-robotics/ros2/qos-executors-time|25.5]]): the publisher offers best effort and the controller requests reliable. A request stricter than the offer is incompatible, so the pair is never matched and not one message is delivered. (iii) `ros2 run tf2_ros tf2_echo` for `map\to cart` ([[04-robotics/ros2/describing-a-robot|25.6]]): the frame does not exist, so no node is broadcasting the cart's transform. Why no crash: an incompatible pair is not an error. Both processes stay healthy, and the only trace is one warning at discovery (“offering incompatible QoS”) in the controller's log. Effort zero with a live encoder is consistent with “no pose, no TF, still running.” A crash would have been kinder.
...
1. **그리기.** P6의 노드 셋: `vision`($50\,\mathrm{Hz}$ 자세), `tf`(카트 프레임), `controller`($200\,\mathrm{Hz}$, 엔코더를 읽고 노력을 쓴다). 토픽 둘과 TF 간선에 이름을 붙이고, $70\,\mathrm{ms}$ 예산을 어느 프로세스가 소유하는지 표시하라.
2. **유도.** 카트에 $0.40\,\mathrm{m}$ 떨어진 목표로 $0.10\,\mathrm{m/s}$로 가라는 명령이 내려진다. (a) 운동은 얼마 동안 이어져야 하는가? (b) 그 구간에 비전 메시지, 제어 틱, 엔코더 카운트는 각각 몇 개인가? (c) `controller`가 비전 스탬프 대신 `time=0`(최신)으로 TF를 찾는다. 비전이 $120\,\mathrm{ms}$ 늦다면 제어기가 실제로 쓰는 나이는 얼마이고, $70\,\mathrm{ms}$ 예산은 지켜지는가?
3. **해석.** 런치 뒤 엔코더는 올라가고 `/tf`는 조용하며 모터 노력은 계속 0이다. 비전 토픽에 `ros2 topic hz`를 걸면 $50\,\mathrm{Hz}$가 나오는데 제어기의 자세 콜백은 한 번도 불리지 않는다. 맨 먼저 할 점검 세 가지를 순서대로 적고, 각각이 이 트랙의 어느 페이지에 있는지 밝혀라. 그중 하나는 QoS 불일치다: `vision`은 센서 데이터 프로파일(best effort)로 발행하고 `controller`는 기본 프로파일(reliable)로 구독한다. 이것이 왜 프로세스가 죽는 대신 “아무 일도 안 일어남”으로 나타나는지 설명하라.

> [!tip]- 정답 · Solutions
> 1. `vision` $\to$ 자세 토픽 $\to$ `controller`; `vision` $\to$ TF `map\to cart`; 엔코더는 로컬 읽기다. 예산은 카메라가 아니라 제어기 프로세스가 소유한다.
> 2. (a) $0.40/0.10=4.0\,\mathrm{s}$. (b) 비전 메시지 $4\times 50=200$개, 틱 $4\times 200=800$개, 카운트 $0.40\times 2048=819$개. (c) 최신 TF는 비전 스탬프보다 *새로울* 수도 있고, TF가 굶으면 *오래될* 수도 있다. 비전이 $120\,\mathrm{ms}$ 늦은데 `time=0`을 쓰면 대개 자세와 다른 순간의 변환을 적용하게 되므로, $70\,\mathrm{ms}$ 예산은 이미 깨졌고($120>70$) 프레임끼리도 어긋난다. 스탬프로 찾아라.
> 3. (i) 비전 토픽에 `ros2 topic list` / `hz` ([[04-robotics/ros2/nodes-topics-messages|25.2]]): $50\,\mathrm{Hz}$이므로 `vision`은 살아 있고, 끊긴 곳은 토픽과 `controller` 사이다. `hz`는 best effort로 구독하므로 reliable 구독자가 받지 못하는 best effort 발행자도 본다. (ii) 비전 토픽에 `ros2 topic info -v` ([[04-robotics/ros2/qos-executors-time|25.5]]): 발행자는 best effort를 제공하고 제어기는 reliable을 요청한다. 요청이 제공보다 엄격하면 비호환이므로 둘은 끝내 짝지어지지 않고 메시지는 하나도 전달되지 않는다. (iii) `map\to cart`에 `ros2 run tf2_ros tf2_echo` ([[04-robotics/ros2/describing-a-robot|25.6]]): 프레임이 없다고 나오므로 카트의 변환을 방송하는 노드가 없다. 죽지 않는 이유: 비호환 쌍은 오류가 아니다. 두 프로세스는 멀쩡하고, 남는 흔적은 발견 시점에 제어기 로그에 찍히는 경고 한 줄(“offering incompatible QoS”)뿐이다. 엔코더가 살아 있는데 노력이 0인 것은 “자세 없음, TF 없음, 프로세스는 도는 중”과 맞다. 차라리 죽었으면 찾기 쉬웠을 것이다.
