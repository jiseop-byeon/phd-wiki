...
That silent staleness has a flag of its own, a third one: `--packages-above` builds the named packages and every package that recursively depends on them, which is exactly the set a changed message definition makes stale. On P6, after a field is added to `CartState.msg`, `colcon build --packages-above p6_interfaces` rebuilds `p6_interfaces`, then `p6_control`, then anything that depends on either, and never `p6_perception`, which depends on neither (§4). The selection flags work on `colcon list` too, so `colcon list --packages-above p6_interfaces --names-only` prints that set before you spend a build on it. Match the flag to the file you edited: an existing `.py` file under `--symlink-install` needs $0$ packages rebuilt (§7), a `.cpp` file in `p6_control` needs $1$, with `--packages-select`, and a `.msg` file in `p6_interfaces` needs everything above it — $2$ of P6's $4$ packages, or $3$ if `p6_bringup`'s `package.xml` declares the controller it launches.
...
- colcon source — [`colcon_package_selection/package_selection/dependencies.py`](https://github.com/colcon/colcon-package-selection/blob/master/colcon_package_selection/package_selection/dependencies.py) (`--packages-above`: the named packages and every package that recursively depends on them); [`colcon_package_information/verb/list.py`](https://github.com/colcon/colcon-package-information/blob/master/colcon_package_information/verb/list.py) (`colcon list` takes the package-selection arguments and `--names-only`).
...
그 조용한 낡음에는 따로 플래그가 있다. 세 번째 플래그인 `--packages-above`는 지정한 패키지와, 그것에 재귀적으로 의존하는 모든 패키지를 빌드한다. 바뀐 메시지 정의가 낡게 만드는 집합이 정확히 그것이다. P6에서 `CartState.msg`에 필드를 하나 더하면, `colcon build --packages-above p6_interfaces`는 `p6_interfaces`를, 이어서 `p6_control`을, 그리고 둘 중 하나에 의존하는 것은 무엇이든 다시 빌드하고, 어느 쪽에도 의존하지 않는 `p6_perception`(4절)은 건드리지 않는다. 선택 플래그는 `colcon list`에도 통하므로, `colcon list --packages-above p6_interfaces --names-only`가 빌드를 쓰기 전에 그 집합을 찍어 준다. 플래그는 고친 파일에 맞춰 고른다. `--symlink-install` 아래의 기존 `.py` 파일이면 다시 빌드할 패키지가 $0$개(7절), `p6_control`의 `.cpp` 파일이면 `--packages-select`로 $1$개, `p6_interfaces`의 `.msg` 파일이면 그 위의 전부다. P6의 패키지 $4$개 가운데 $2$개이고, `p6_bringup`의 `package.xml`이 자기가 띄우는 제어기를 선언한다면 $3$개다.
...
- colcon 소스 — [`colcon_package_selection/package_selection/dependencies.py`](https://github.com/colcon/colcon-package-selection/blob/master/colcon_package_selection/package_selection/dependencies.py)(`--packages-above`: 지정한 패키지와 그것에 재귀적으로 의존하는 모든 패키지), [`colcon_package_information/verb/list.py`](https://github.com/colcon/colcon-package-information/blob/master/colcon_package_information/verb/list.py)(`colcon list`는 패키지 선택 인자와 `--names-only`를 받는다).
