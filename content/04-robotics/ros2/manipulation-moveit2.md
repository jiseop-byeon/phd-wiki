...
On P6 the difference is a number. Servo's loop runs once per `publish_period`, $0.01\,\mathrm{s}$ by default ($100\,\mathrm{Hz}$), turning the latest command into a joint command each time, so each output spans $0.01/0.005=2$ ticks of the $200\,\mathrm{Hz}$ loop underneath. Fed the panel pose from vision, it steers towards a target at most one frame old: in $T_v=20\,\mathrm{ms}$ the cart at the page-local $v$ moves $0.25\times0.020=5.0\,\mathrm{mm}$, the first row of the Worked case's Step 2 table, whose second row puts a returning plan $205\,\mathrm{mm}$ behind, $41$ times as far. The price is a freshness check on the command's own stamp. Servo discards a command whose age — its clock's `now()` minus the command's `header.stamp` — has reached `incoming_command_timeout`, $0.1\,\mathrm{s}$ by default, and brings the arm to a smooth halt. That is $5$ vision periods, so a camera that drops out stops the arm once its last frame is $0.1\,\mathrm{s}$ old, after $25\,\mathrm{mm}$ of cart travel, which is the behaviour you want. But a vision node that stamps at mid-exposure and delivers $200\,\mathrm{ms}$ late sends only commands that are stale on arrival, and the arm never moves, while one inside P6's $70\,\mathrm{ms}$ budget arrives with $30\,\mathrm{ms}$ to spare. A command whose stamp was left at zero is, on the wall clock, decades old: it never moves the arm at all.
...
*In your own process* has a consequence worth stating. `MoveItCpp` builds its own planning scene monitor and its own trajectory execution manager on your node, which is why that node must carry the whole configuration (§6), and why §8's execution gate is the same gate from Python: `allowed_start_tolerance` still defaults to $0.01$, a centimetre on P6's prismatic cart joint, which the cart crosses in $40\,\mathrm{ms}$ (Worked case, Step 3), so a Python prototype on a moving base is refused exactly as a C++ one is. And the `controllers=[]` in the snippet does not mean *no controller*: given an empty list, the execution manager considers every controller in §8's YAML and chooses ones that cover the trajectory's joints, preferring those marked `default: true`, then those with fewer joints.

The missing Cartesian binding has a workaround that fails differently. From Python a straight line is a Pilz request: a `PlanRequestParameters` whose `planning_pipeline` is `pilz_industrial_motion_planner` and whose `planner_id` is `LIN` (§5), passed as `plan(single_plan_parameters=...)`. On the Worked case's $0.20\,\mathrm{m}$ insertion, C++'s `computeCartesianPath` can return $0.62$ of the line and leave the tool $76\,\mathrm{mm}$ short inside the slot; `LIN` returns the whole line or an error, never a part. Check the planner id if you copy the configuration of the official Python tutorial: its block named `pilz_lin` sets `planner_id: "PTP"`, a joint-space motion whose tool path is not a straight line.
...
- `moveit/moveit2` (branch `jazzy`) — `servo_parameters.yaml` (the `publish_period` and `incoming_command_timeout` defaults): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/moveit_servo/config/servo_parameters.yaml); `servo_node.cpp` (the loop at `1/publish_period`, and the stamp-based staleness check that ends in a smooth halt): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/moveit_servo/src/servo_node.cpp); `moveit_cpp.cpp` (its own planning scene monitor and trajectory execution manager on the caller's node): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/planning/moveit_cpp/src/moveit_cpp.cpp); `moveit_py` `planning_component.cpp` (`PlanRequestParameters` fields, `plan(single_plan_parameters=...)`, no Cartesian-path binding): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_py/src/moveit/moveit_ros/moveit_cpp/planning_component.cpp); and `trajectory_execution_manager.cpp` again for the controller choice from an empty list. `moveit/moveit2_tutorials` (branch `main`) — `motion_planning_python_api_tutorial.yaml` (the `pilz_lin` block with `planner_id: "PTP"`): [GitHub](https://github.com/moveit/moveit2_tutorials/blob/main/doc/examples/motion_planning_python_api/config/motion_planning_python_api_tutorial.yaml).
...
P6에서는 그 차이가 숫자다. Servo의 루프는 `publish_period`마다, 기본 $0.01\,\mathrm{s}$($100\,\mathrm{Hz}$)마다 한 번 돌며 가장 최근 명령을 관절 명령으로 바꾸므로, 출력 하나가 밑에서 도는 $200\,\mathrm{Hz}$ 루프의 $0.01/0.005=2$틱에 걸친다. 비전이 준 패널 자세를 명령으로 넣으면 길어야 한 프레임 묵은 목표를 향해 조향한다. $T_v=20\,\mathrm{ms}$ 동안 페이지 국소 $v$의 카트는 $0.25\times0.020=5.0\,\mathrm{mm}$ 움직이는데, 이것이 Worked case Step 2 표의 첫 행이고, 그 둘째 행에서 계획은 돌아오는 순간 이미 그 $41$배인 $205\,\mathrm{mm}$ 뒤처져 있다. 대가는 명령 자신의 스탬프로 재는 신선도 검사다. Servo는 명령의 나이, 곧 자기 시계의 `now()`에서 명령의 `header.stamp`를 뺀 값이 `incoming_command_timeout`(기본 $0.1\,\mathrm{s}$)에 이르면 그 명령을 버리고 팔을 부드럽게 멈춘다. 비전 $5$주기다. 그래서 카메라가 끊기면 마지막 프레임이 $0.1\,\mathrm{s}$ 묵는 순간, 카트가 $25\,\mathrm{mm}$ 간 뒤에 팔이 멈추고, 그것이 바라던 동작이다. 그러나 노출 중간에 스탬프를 찍고 $200\,\mathrm{ms}$ 늦게 전달하는 비전 노드는 도착하는 순간 이미 낡은 명령만 보내므로 팔은 끝내 움직이지 않고, P6의 $70\,\mathrm{ms}$ 예산 안에 드는 노드는 $30\,\mathrm{ms}$의 여유를 두고 도착한다. 스탬프를 0으로 둔 명령은 wall clock으로 수십 년 묵은 것이어서 팔을 한 번도 움직이지 못한다.
...
*당신 프로세스 안에서*라는 말에는 짚어 둘 결과가 있다. `MoveItCpp`는 당신 노드 위에 자기 planning scene monitor와 자기 trajectory execution manager를 따로 세운다. 그래서 그 노드가 설정 전체를 들고 있어야 하고(§6), Python에서도 §8의 실행 관문은 같은 관문이다. `allowed_start_tolerance`는 여전히 기본 $0.01$이고, P6의 직동 카트 관절에서는 1센티미터여서 카트가 $40\,\mathrm{ms}$ 만에 지나간다(Worked case Step 3). 움직이는 베이스 위의 Python 시제품은 C++ 것과 똑같이 거부된다. 그리고 코드의 `controllers=[]`는 *제어기 없음*이 아니다. 빈 목록을 받은 execution manager는 §8 YAML의 제어기를 모두 후보로 놓고 궤적의 관절을 덮는 것을 고르는데, `default: true`로 표시된 것을 먼저, 그다음 관절 수가 적은 것을 고른다.

Cartesian path 바인딩이 없는 자리에는 우회로가 있고, 그것은 다르게 실패한다. Python에서 직선은 Pilz 요청이다. `planning_pipeline`이 `pilz_industrial_motion_planner`이고 `planner_id`가 `LIN`인 `PlanRequestParameters`(§5)를 `plan(single_plan_parameters=...)`로 넘긴다. Worked case의 $0.20\,\mathrm{m}$ 삽입에서 C++의 `computeCartesianPath`는 선의 $0.62$만 돌려주고 공구를 슬롯 안에서 $76\,\mathrm{mm}$ 모자란 곳에 세워 둘 수 있지만, `LIN`은 선 전체를 돌려주거나 오류를 낼 뿐 일부만 주는 일이 없다. 공식 Python 튜토리얼의 설정을 베낀다면 planner id를 확인하라. `pilz_lin`이라는 이름의 블록이 `planner_id: "PTP"`를 설정하는데, 이는 관절 공간 동작이어서 공구 경로가 직선이 아니다.
...
- `moveit/moveit2`(`jazzy` 브랜치) — `servo_parameters.yaml`(`publish_period`와 `incoming_command_timeout`의 기본값): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/moveit_servo/config/servo_parameters.yaml), `servo_node.cpp`(`1/publish_period`로 도는 루프, 스탬프로 재는 신선도 검사와 그 끝의 부드러운 정지): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/moveit_servo/src/servo_node.cpp), `moveit_cpp.cpp`(호출한 노드 위에 따로 세우는 planning scene monitor와 trajectory execution manager): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_ros/planning/moveit_cpp/src/moveit_cpp.cpp), `moveit_py`의 `planning_component.cpp`(`PlanRequestParameters` 필드, `plan(single_plan_parameters=...)`, Cartesian path 바인딩 없음): [GitHub](https://github.com/moveit/moveit2/blob/jazzy/moveit_py/src/moveit/moveit_ros/moveit_cpp/planning_component.cpp), 그리고 빈 목록에서의 제어기 선택은 다시 `trajectory_execution_manager.cpp`. `moveit/moveit2_tutorials`(`main` 브랜치) — `motion_planning_python_api_tutorial.yaml`(`planner_id: "PTP"`를 둔 `pilz_lin` 블록): [GitHub](https://github.com/moveit/moveit2_tutorials/blob/main/doc/examples/motion_planning_python_api/config/motion_planning_python_api_tutorial.yaml).
