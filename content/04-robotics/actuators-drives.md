...
> Plant **P2**, the catalog's planar two-link arm, from [[02-foundations/lab-plants|0.6 Lab Plants]] (*plant*: control's word for the system being controlled), with its mass matrix, gravity torque and tip mass from [[02-foundations/manipulator-kinematics-dynamics|foundations 10 §3, §5 and §6]], and the mass matrix read as kinetic energy in [[04-robotics/modern-robotics/ch08-dynamics|MR ch.8 §2]]. The physics floor under the drive: power and the energy ledger, the holding torque, and gears with their $n^2J_m$ from [[02-foundations/basic-mechanics|0.6.1 Basic Mechanics §6, §8 and §9]]; $i^2R$ heat, Kirchhoff's voltage law, the inductor and its time constant, the pulse-width-modulation (PWM) bridge and the motor as a circuit from [[02-foundations/basic-circuits-electronics|0.6.2 Basic Circuits & Electronics §2, §3, §4, §6 and §7]]. The integrators of [[02-foundations/lab-kernel|0.7 Lab Kernel]], and the quintic time scaling of [[04-robotics/modern-robotics/ch09-trajectory-generation|MR ch.9 §1]], which the lab's lift follows. Second-order parameters, bandwidth, and PID with anti-windup from [[04-robotics/control-theory-ce397|5. Control Theory §5, §5.5 and §7]]. The torque and back-EMF constants, the torque–speed line, the gear ratio with its efficiency, reflected inertia and backdrivability are defined on this page (§1–§4, §7), and 0.6.1 and 0.6.2 defer them here, so no later page is needed; the capstan version of the gear trade, on a haptic handle, is optional further reading in the haptics track.
> 장치 **P2**, 곧 카탈로그의 평면 2링크 팔([[02-foundations/lab-plants|0.6 Lab Plants]]. *장치*(plant)는 제어가 다루는 대상 시스템을 부르는 말이다), 그리고 [[02-foundations/manipulator-kinematics-dynamics|기초 10 §3, §5, §6]]의 질량 행렬·중력 토크·말단 질량과, 질량 행렬을 운동 에너지로 읽는 [[04-robotics/modern-robotics/ch08-dynamics|MR 8장 §2]]. 구동계 밑의 물리 층: [[02-foundations/basic-mechanics|0.6.1 기초 역학 §6, §8, §9]]의 일률과 에너지 장부, 유지 토크, 기어와 그 $n^2J_m$, 그리고 [[02-foundations/basic-circuits-electronics|0.6.2 기초 회로와 전자 §2, §3, §4, §6, §7]]의 $i^2R$ 열, 키르히호프 전압 법칙, 인덕터와 그 시정수, 펄스 폭 변조(PWM) 브리지, 회로로 본 모터. [[02-foundations/lab-kernel|0.7 Lab Kernel]]의 적분기와, 랩의 들어올리기가 따르는 [[04-robotics/modern-robotics/ch09-trajectory-generation|MR 9장 §1]]의 5차 시간 스케일링. [[04-robotics/control-theory-ce397|5. 제어 이론 §5, §5.5, §7]]의 2차계 파라미터, 대역폭, anti-windup이 있는 PID. 토크 상수와 역기전력 상수, 토크–속도 선, 효율을 가진 감속비, 반사 관성, 역구동성은 이 페이지(§1–§4, §7)에서 정의하고 0.6.1과 0.6.2도 여기로 미루므로 뒤쪽 페이지는 필요 없다. 기어 맞바꿈의 캡스턴 판(햅틱 손잡이 위)은 햅틱 트랙에서 골라 더 읽을 거리다.

## English

*Stands on [[02-foundations/manipulator-kinematics-dynamics|foundations 10. Manipulator Kinematics & Dynamics]] for P2's $M$ and $g$, on the physics floor's [[02-foundations/basic-mechanics|0.6.1 Basic Mechanics]] and [[02-foundations/basic-circuits-electronics|0.6.2 Basic Circuits & Electronics]] for gears, power and the motor as a circuit, and on [[04-robotics/control-theory-ce397|5. Control Theory]] for PD, bandwidth and anti-windup. Sits beside [[04-robotics/robot-systems-deployment|10. Robot Systems §2]], which lists actuators as part of embodiment without opening one. A later use of plant **P2**, now with a motor and a gearbox at each joint.*

> [!note] Why this matters · 왜 배우는가
> This page sits in the systems column beside the robot stack of [[07-research-program/index|7. Research Program §5]], directly on the physics floor of [[02-foundations/basic-mechanics|0.6.1]] and [[02-foundations/basic-circuits-electronics|0.6.2]]: it opens the drive through which the manipulation layer acts, and in *"install that panel on the frame"* it serves *move the component* — every torque a planner or controller asks of a joint must come out of a winding as current, voltage and heat — and, where §7 reads motor current as joint torque, *detect contact*; its chip sits in the systems column of the [[physical-ai-map|Physical AI Map]]. Without it a joint is judged by a demo: holding **P2**, the catalog's planar two-link arm ([[02-foundations/lab-plants|0.6]]), at its catalog pose costs $2.45\,\mathrm A$ and a $60.1\,\mathrm K$ winding rise on the $n=100$ drive, but the same hold on an $n=50$ drive passes a ten-second demo and reaches its thermal limit at $32.2\,\mathrm s$ (§6), and on an $n=300$ drive reading current as torque overstates the lift's peak by $93\,\%$ (§7). [[04-robotics/force-compliance-control|13. Force & Compliance Control]] leans on it in §2 (torque-based impedance wants a backdrivable joint) and §5 (the rotor's $n^2J_m$ in the impact mass), and [[04-robotics/teleoperation-demonstration|12. Teleoperation & Demonstration §4.5]] on its reflected inertia — block 2 of the dissertation path ([[07-research-program/index|7. Research Program §8]], robotics sessions 90–94), with 12 and 13 following in block 3. After it you can turn any joint torque into amps, volts, watts and kelvin, place a hold and a move on a drive's torque–speed region, and ask of any arm or paper whether its torque is continuous or peak and whether its current can be read as force.

> [!note] First pass · 처음이라면
> About five 60–90-minute sessions, robotics 90–94. **Session 1:** the Running object, the picture and the Worked case by hand, following the drive chain drawn at its start — one joint torque turned into amps, volts, watts and kelvin, and one entry of P2's mass matrix changed by a rotor you never see. End by redoing Steps 2–5 at $n=80$ with the answers covered ($3.07\,\mathrm A$, $94\,\mathrm K$: it still holds). **Session 2:** §1–§3 — the two motor equations, the gearbox, the torque–speed line. **Session 3:** §4–§6 — reflected inertia, the two time constants, heat; the copper note of §6 is second pass. **Session 4:** §8, the lab, and problem 3. **Session 5:** §7, backdrivability, which reads the lab's lift back from the motor current (its impact-mitigation note is second pass), then §9, the self-check and problems 1–2.

### Running object · 이 페이지의 대상

**P2** from [[02-foundations/lab-plants|0.6 Lab Plants]], standing in the vertical plane with $g=9.81\,\mathrm{m/s^2}$ along $-y$, at its frozen pose $\theta=(0^\circ,90^\circ)$: elbow at $(1,0)$, forearm straight up, tip at $(1,1)\,\mathrm{m}$. Two of its numbers carry the whole page, both derived on [[02-foundations/manipulator-kinematics-dynamics|foundations 10 §3 and §5]]: the shoulder's mass-matrix entry $M_{11}=3\,\mathrm{kg{\cdot}m^2}$ and the shoulder's gravity torque $g_1=19.62\,\mathrm{N{\cdot}m}$. The elbow's gravity torque is $0$ at this pose, because the forearm mass sits directly above the elbow.

Each joint now gets a **drive**: a DC motor, the amplifier that feeds it (a PWM bridge, [[02-foundations/basic-circuits-electronics|0.6.2 §6]]), and a gearbox between the motor and the link. The catalog has no drive, so this page specifies one completely and freezes it. The values are **illustrative** — the right order of magnitude for a small arm joint, chosen for this page, and not taken from any product's datasheet.
...
<svg viewBox="0 0 560 284" style="max-width:100%;height:auto" role="img" aria-label="joint torque against joint speed for P2's shoulder drive at gear ratio 100: current limit 80 N m, voltage line from 1.4 to 2.4 rad/s, continuous thermal line at 25.3 N m, the hold point H at 19.62 N m, and the lift's demand loop from its start at 21.9 N m up to 39.1 N m and back to H">
  ...
  <circle cx="70" cy="196.2" r="3.2" fill="none" stroke="currentColor" stroke-width="1.3"/>
  <g font-size="11" fill="currentColor">
    <text x="540" y="22" text-anchor="end" xml:space="preserve">P2 shoulder drive · n = 100 · V<tspan dy="3" font-size="10">s</tspan><tspan dy="-3"> = 24 V</tspan></text>
    <text x="12" y="40" xml:space="preserve">joint torque η·n·k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3">·i (N·m)</tspan></text>
    ...
    <text x="290" y="206" font-size="10.5">continuous, 25.3 N·m</text>
    <text x="100" y="151">the lift: 0.5 rad in 0.8 s</text>
    <text x="84" y="216">← H: hold, 19.62 N·m, 2.45 A</text>
    <text x="64" y="190" text-anchor="end" font-size="10.5">start 21.9</text>
  </g>
</svg>


The operating region of P2's shoulder on the $n=100$ drive, in joint units: the current limit flat at $80\,\mathrm{N{\cdot}m}$, the voltage line $\tau=192-80\,\dot\theta$ falling from its corner at $1.4\,\mathrm{rad/s}$ to the no-load speed $2.4\,\mathrm{rad/s}$, and the dashed thermal line at the continuous torque $25.3\,\mathrm{N{\cdot}m}$, above which the drive can stay only for seconds. The hold point H, $19.62\,\mathrm{N{\cdot}m}$ at $2.45\,\mathrm{A}$, sits under the thermal line, and the lift of §8 — the shoulder raised $0.5\,\mathrm{rad}$ in $0.8\,\mathrm{s}$ — traces a loop that starts at rest at $21.9\,\mathrm{N{\cdot}m}$, gravity at $-0.5\,\mathrm{rad}$, climbs to $39.1\,\mathrm{N{\cdot}m}$ while it accelerates, passes $1.17\,\mathrm{rad/s}$ at $21.5\,\mathrm{N{\cdot}m}$ at mid-move, and ends at H without leaving the region.

### Worked case · 대상으로 한 번 끝까지

Hold P2 at its frozen pose through the $n=100$ drive, and follow one number, the shoulder's gravity torque, down to the winding and back out as heat. Then see what the drive did to the mass matrix. Six steps, along the chain drawn here.

<svg viewBox="0 0 560 292" style="max-width:100%;height:auto" role="img" aria-label="P2's shoulder drive as a chain of five stages: supply and amplifier, winding, rotor, gearbox and link. For the hold, 2.45 V drives 2.45 A, which makes 0.2453 N·m at the rotor, multiplied by 80 in the gearbox to the link's 19.62 N·m, while 6.01 W of heat through 10 K/W raises the winding 60.1 K. For the lift at 0.40 s, 42.5 W leaves the supply, 8.0 W heats the winding and 0.1 W returns from the inductance, 34.6 W crosses the back-EMF, 0.5 W goes to the rotor and bearings, 34.1 W enters the gearbox, 6.8 W is lost there and 27.3 W reaches the link.">
  <defs><marker id="dcA" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor"/></marker></defs>
  <g fill="currentColor">
    <text x="12" y="18" font-size="11.5" font-weight="600" fill-opacity="0.85">the drive chain: the hold (Steps 1–5) and the lift's power at t = 0.40 s (§8)</text>
    <path d="M 10.0 38 L 10.0 32 L 216.0 32 L 216.0 38" fill="none" stroke="currentColor" stroke-width="0.8" stroke-opacity="0.55"/>
    <text x="113.0" y="29" font-size="10" text-anchor="middle" fill-opacity="0.75" font-style="italic">electrical</text>
    <path d="M 230.0 38 L 230.0 32 L 546.0 32 L 546.0 38" fill="none" stroke="currentColor" stroke-width="0.8" stroke-opacity="0.55"/>
    <text x="388.0" y="29" font-size="10" text-anchor="middle" fill-opacity="0.75" font-style="italic">mechanical</text>
    <rect x="10.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="58" y="59" font-size="11" font-weight="600" text-anchor="middle">supply, amplifier</text>
    <text x="58" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">V<tspan dy="3" font-size="10">s</tspan><tspan dy="-3"> = 24 V</tspan></text>
    <text x="58" y="88" font-size="10.5" text-anchor="middle" xml:space="preserve">I<tspan dy="3" font-size="10">max</tspan><tspan dy="-3"> = 10 A</tspan></text>
    <rect x="120.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="168" y="59" font-size="11" font-weight="600" text-anchor="middle">winding</text>
    <text x="168" y="74" font-size="10.5" text-anchor="middle">R = 1.0 Ω</text>
    <text x="168" y="88" font-size="10.5" text-anchor="middle">L = 1.0 mH</text>
    <rect x="230.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="278" y="59" font-size="11" font-weight="600" text-anchor="middle">rotor</text>
    <text x="278" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">J<tspan dy="3" font-size="10">m</tspan><tspan dy="-3"> = 10⁻⁴ kg·m²</tspan></text>
    <text x="278" y="88" font-size="10.5" text-anchor="middle" xml:space="preserve">k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3"> = 0.10 N·m/A</tspan></text>
    <rect x="340.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="388" y="59" font-size="11" font-weight="600" text-anchor="middle">gearbox</text>
    <text x="388" y="74" font-size="10.5" text-anchor="middle">n = 100</text>
    <text x="388" y="88" font-size="10.5" text-anchor="middle">η = 0.80</text>
    <rect x="450.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="498" y="59" font-size="11" font-weight="600" text-anchor="middle">link: P2 shoulder</text>
    <text x="498" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">M<tspan dy="3" font-size="10">11</tspan><tspan dy="-3"> = 3 kg·m²</tspan></text>
    <text x="498" y="88" font-size="10.5" text-anchor="middle">at (0°, 90°)</text>
    <line x1="107.0" y1="69.0" x2="119.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcA)"/>
    <line x1="217.0" y1="69.0" x2="229.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcA)"/>
    <line x1="327.0" y1="69.0" x2="339.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcA)"/>
    <line x1="437.0" y1="69.0" x2="449.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcA)"/>
    <path d="M 58 96 L 58 104 M 58 119 L 58 164 M 58 178 L 58 188 M 58 203 L 58 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 168 96 L 168 104 M 168 119 L 168 164 M 168 178 L 168 188 M 168 203 L 168 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 278 96 L 278 104 M 278 119 L 278 164 M 278 178 L 278 188 M 278 203 L 278 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 388 96 L 388 104 M 388 119 L 388 164 M 388 178 L 388 188 M 388 203 L 388 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 498 96 L 498 104 M 498 119 L 498 164 M 498 178 L 498 188 M 498 203 L 498 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <text x="12" y="114" font-size="10.5" font-style="italic" fill-opacity="0.85">the hold: 19.62 N·m followed back to the winding and out as heat</text>
    <text x="62" y="131" font-size="11" font-weight="600">2.45 V</text>
    <text x="62" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">of 24 V</text>
    <text x="172" y="131" font-size="11" font-weight="600">2.45 A</text>
    <text x="172" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">6.01 W of heat</text>
    <text x="282" y="131" font-size="11" font-weight="600">0.2453 N·m</text>
    <text x="282" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">= k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3"> i</tspan></text>
    <text x="392" y="131" font-size="11" font-weight="600">× ηn = 80</text>
    <text x="392" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">not × 100</text>
    <text x="502" y="131" font-size="11" font-weight="600">19.62 N·m</text>
    <text x="502" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">gravity, g₁</text>
    <line x1="172" y1="150" x2="172" y2="164" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcA)"/>
    <text x="178" y="174" font-size="10.5" xml:space="preserve">thermal: R<tspan dy="3" font-size="10">th</tspan><tspan dy="-3"> = 10 K/W</tspan>, so ΔT = 60.1 K</text>
    <text x="12" y="198" font-size="10.5" font-style="italic" fill-opacity="0.85">the lift at t = 0.40 s: 1.226 rad/s, 2.824 A, 15.05 V — watt by watt</text>
    <circle cx="58" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="168" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="278" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="388" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="498" cy="224" r="3.5" fill="currentColor"/>
    <line x1="64" y1="224" x2="162" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcA)"/>
    <text x="113.0" y="218" font-size="11" font-weight="600" text-anchor="middle">42.5 W</text>
    <line x1="174" y1="224" x2="272" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcA)"/>
    <text x="223.0" y="218" font-size="11" font-weight="600" text-anchor="middle">34.6 W</text>
    <line x1="284" y1="224" x2="382" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcA)"/>
    <text x="333.0" y="218" font-size="11" font-weight="600" text-anchor="middle">34.1 W</text>
    <line x1="394" y1="224" x2="492" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcA)"/>
    <text x="443.0" y="218" font-size="11" font-weight="600" text-anchor="middle">27.3 W</text>
    <line x1="168" y1="229" x2="168" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcA)"/>
    <text x="172" y="258" font-size="10.5">8.0 W heat,</text>
    <text x="172" y="271" font-size="10.5" fill-opacity="0.85">0.1 W back from L</text>
    <line x1="278" y1="229" x2="278" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcA)"/>
    <text x="282" y="258" font-size="10.5">0.5 W rotor,</text>
    <text x="282" y="271" font-size="10.5" fill-opacity="0.85">bearings</text>
    <line x1="388" y1="229" x2="388" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcA)"/>
    <text x="392" y="258" font-size="10.5">6.8 W lost</text>
    <text x="392" y="271" font-size="10.5" fill-opacity="0.85">(1 − η)</text>
    <text x="458" y="246" font-size="10.5">lifts and speeds</text>
    <text x="458" y="259" font-size="10.5">the arm: 64 %</text>
  </g>
</svg>

The drive of P2's shoulder as one chain from supply to link, in electrical and mechanical stages, with heat as the third domain. Top: the hold of Steps 1–5 — the link's $19.62\,\mathrm{N{\cdot}m}$ is $\eta n=80$ times the rotor's $0.2453\,\mathrm{N{\cdot}m}$, which takes $2.45\,\mathrm A$ at $2.45\,\mathrm V$ of the $24\,\mathrm V$ supply, and the $6.01\,\mathrm W$ of heat settles the winding $60.1\,\mathrm K$ above ambient through $R_{th}$. Bottom: the same chain carrying power at $t=0.40\,\mathrm s$ of §8's lift — $42.5\,\mathrm W$ from the supply, $8.0\,\mathrm W$ of heat and $0.1\,\mathrm W$ back from the inductance, $34.6\,\mathrm W$ across the back-EMF, $0.5\,\mathrm W$ to the rotor and bearings, $34.1\,\mathrm W$ into the gearbox, $6.8\,\mathrm W$ lost there and $27.3\,\mathrm W$, $64\,\%$ of the supply, delivered to the link.

**Step 1 — what the joint must supply.** At rest the manipulator equation of [[02-foundations/manipulator-kinematics-dynamics|foundations 10 §5]] leaves $\tau=g(\theta)$, so the shoulder must supply its holding torque, $g_1=19.62\,\mathrm{N{\cdot}m}$, and the elbow nothing — the statics of [[02-foundations/basic-mechanics|0.6.1 §8]] gives the same two numbers from a free-body diagram.

**Step 2 — through the gearbox.** With the motor driving, the gearbox multiplies torque by $\eta n$ (§2; the lossless lever-and-gear trade is [[02-foundations/basic-mechanics|0.6.1 §9]]), so the motor must make
...
**Step 4 — the voltage.** The terminal voltage splits three ways by Kirchhoff's voltage law ([[02-foundations/basic-circuits-electronics|0.6.2 §3]]), $V=Ri+L\,di/dt+k_e\omega_m$: the resistance's drop, the inductance's drop ([[02-foundations/basic-circuits-electronics|0.6.2 §4]]) and the back-EMF of the spinning rotor (§1). Nothing turns, so there is no back-EMF, and the current is steady, so there is no inductive drop; that leaves only $V=Ri=2.45\,\mathrm{V}$ of the $24\,\mathrm{V}$ supply.

**Step 5 — the heat.** The supply delivers $Vi=6.01\,\mathrm{W}$ and the arm does no work — holding still costs no mechanical power ([[02-foundations/basic-mechanics|0.6.1 §6]]) — so every one of those watts is heat in the winding's resistance, $P=i^2R=2.4525^2\cdot1.0=6.01\,\mathrm{W}$ ([[02-foundations/basic-circuits-electronics|0.6.2 §2]]). Through the thermal resistance the winding settles at
...
where the $0.0001$ is the elbow rotor riding along on link 1. Every entry is kinetic energy per squared joint rate ([[04-robotics/modern-robotics/ch08-dynamics|MR ch.8 §2]]), so the gearbox's losses do not appear in it. A rotor with $1/30{,}000$ of the shoulder's inertia has become a third of it, and at the elbow the drive has doubled the arm. Counted as the inertia the motor has to push through the lossy gearbox — still in joint units — the shoulder is heavier still:

$$J_{eq}=n^2J_m+\frac{M_{11}}{\eta}=1.0+\frac{3}{0.8}=1.0+3.75=4.75\ \mathrm{kg{\cdot}m^2}$$

since only the link's share passes through the gearbox and pays its $\eta$ (§2 derives it). That is more than the mass matrix's $4.0001$ because the two answer different questions: the mass matrix counts stored energy, and $J_{eq}$ is what the motor must push.
...
To turn the Worked case's $19.62\,\mathrm{N{\cdot}m}$ into amps and volts you need the motor's two equations. A DC motor is one electrical circuit and one rotating body, coupled by two constants; [[02-foundations/basic-circuits-electronics|0.6.2 §7]] met the circuit as an element, and this section adds the rotor and the two constants. Write the circuit with Kirchhoff's voltage law ([[02-foundations/basic-circuits-electronics|0.6.2 §3]]), because the voltage the amplifier puts across the terminals has only three places to go — the winding's resistance, the change of current through its inductance, and the voltage the spinning rotor induces:
...
A $100{:}1$ gearbox does not multiply torque by 100: it multiplies it by $\eta n=80$, because a real gearbox takes its cut of the power, and the motor feels the link only through the ratio — its torque divided by $\eta n$, its inertia by $\eta n^2$. This section derives both. A lever or a gear trades force for distance, and the reflected inertia $n^2J_m$ follows from the rotor's kinetic energy; [[02-foundations/basic-mechanics|0.6.1 Basic Mechanics §9]] works both on P2's gearbox at the lossless ideal. This section adds the gearbox's efficiency and the load as the motor sees it.
...
That equation assumes power flows from the motor to the link; the note below checks that it does throughout §8's lift.

> [!note]- Deeper · 더 깊이
> **One regime condition.** This is the equation of the motor *driving* the load. In §8's lift the link torque $M_{11}\ddot\theta_1+g_1$ stays positive while the arm rises — its smallest value in any row is $0.86\,\mathrm{N{\cdot}m}$, at $n=30$ — so power flows from motor to link for the whole rise, the direction the equation describes. The only reverse flow is the settle-back after each row's small overshoot at the end of the move, where the equation puts the loss on the wrong side; §8 says what that changes.
...
because the gearbox multiplies the motor's torque by $\eta n$ and divides its speed by $n$. Three things move with $n$, and not at the same rate. The current limit $\eta nk_tI_{\max}$ rises as $n$; the no-load speed $V_s/(k_en)$ falls as $1/n$; and the slope of the voltage line, $\eta n^2k_tk_e/R$, steepens as $n^2$. On the $n=100$ drive those are $80\,\mathrm{N{\cdot}m}$, $2.4\,\mathrm{rad/s}$ and $80\,\mathrm{N{\cdot}m}$ per rad/s — the line $\tau_j=192-80\,\dot\theta$ of the picture at the top of the page. Double the ratio and the region doubles in height and halves in width: $160\,\mathrm{N{\cdot}m}$ and $1.2\,\mathrm{rad/s}$ at $n=200$. This is the trade [[04-robotics/modern-robotics/ch09-trajectory-generation|MR ch.9]]'s problem set states in a planner's words when it gives P2's elbow a faster, weaker drive, planned at $v_{\max}=1.6\,\mathrm{rad/s}$ and $a_{\max}=0.5\,\mathrm{rad/s^2}$ against the frozen $0.8$ and $2$: a lower ratio raises the joint's no-load speed as $1/n$ and lowers its current-limited torque as $n$ — halving $n$ doubles the one and halves the other. A planner's limits are chosen inside these physical ones, not on them, which is why that specification can double the speed and still cut the acceleration to a quarter rather than a half.
...
The rotor is tiny and $n^2$ is huge: in the Worked case's Step 6 a rotor with $1/30{,}000$ of the shoulder's inertia became a third of it. This section defines that reflected inertia, finds the ratio past which it outweighs the arm, and says what that does to the arm's dynamics.
...
That is what the rotor adds to $M_{11}$, exactly and with no efficiency in it, because the mass matrix is kinetic energy and friction stores none. *Modern Robotics* §8.9.2 calls it apparent inertia and derives it the same way; the capstan version is optional further reading in [[04-robotics/haptics-teleoperation/device-design-kinematics|24.3 §3]]. The same $n^2$ governs an excavator: the $600\,\mathrm{kg}$ arm of [[02-foundations/fluid-power|0.6.3 Fluid Power §7]], moving $6$ times as fast as its cylinder rod, is felt at the rod as $6^2\times600=21\,600\,\mathrm{kg}$.
...
**Why it can dominate.** The contest between rotor and arm is one ratio,
...
**Moves are judged by their RMS.** The lift's loop in the picture at the top of the page spends a good part of its length above the thermal line, and that is allowed: heat integrates $i^2$, so over a duty cycle — here the repeating pattern of work and rest, not the switching fraction of [[02-foundations/basic-circuits-electronics|0.6.2 §6]]'s PWM — short against $\tau_{th}$ what the thermal line bounds is the root-mean-square current, $I_{\text{rms}}=\sqrt{\tfrac1T\int_0^Ti^2\,dt}\le I_{\text{cont}}$. The lift's demand, repeated back to back, has $I_{\text{rms}}=3.11\,\mathrm{A}$, which is $98\%$ of continuous; add a $4.2\,\mathrm{s}$ hold after each lift and the five-second cycle falls to $2.57\,\mathrm{A}$.

**On the site's scale.** S1, the construction track's $20\,\mathrm{kg}$ facade panel ([[05-construction-robotics/site-engineering|2.5]]), held at P2's tip needs $19.62+20\cdot9.81\cdot1=215.8\,\mathrm{N{\cdot}m}$ at the shoulder ([[02-foundations/basic-mechanics|0.6.1 §8]]), $8.5$ times this drive's continuous $25.3\,\mathrm{N{\cdot}m}$: a different class of drive, not a longer demo.

The rest of this page holds $R$ fixed; the note below shows what a hot winding's rising resistance adds to the hold.

> [!note]- Deeper · 더 깊이
> **The resistance moves with the heat.** Copper's resistance rises about $0.39\%$ per kelvin ([[02-foundations/basic-circuits-electronics|0.6.2 §2]]), so a hot winding makes more heat at the same current. Folding that into the Worked case, the hold's rise solves $\Delta T=R_{th}i^2R_0(1+\alpha\Delta T)$, so $\Delta T=60.15/(1-0.0039\cdot60.15)=78.6\,\mathrm{K}$ — still inside the budget, but a rise $31\%$ larger than the constant-$R$ estimate.

### 7. Backdrivability at a geared joint

Whether a person can move the arm by hand, how hard an impact is, and whether motor current can be read as joint torque all turn on one property of the transmission — how easily a push at the output moves it — and this section measures it at P2's geared shoulder.
...
> - **Why it matters**: it answers the three questions above; the paragraph after the bullets takes the third.
...
>
> where $\xi=1$ means the free dynamics remove the whole impulse and $\xi\to0$ as $\Lambda\to\Lambda_L$, the joints behaving as if locked; $\Lambda$ and $\Lambda_L$ are operational-space inertias in the sense of [[02-foundations/manipulator-kinematics-dynamics|foundations 10 §6]], matrices over the contact's directions and plain masses for a single direction.
...
- **Speed runs out at the top.** The joint's no-load speed $\omega_0/n$ falls toward the lift's $1.17\,\mathrm{rad/s}$, and with the $Ri$ drop the supply is first touched at $n=178$. At $n=200$ the loop rides the $24\,\mathrm{V}$ limit for $0.21\,\mathrm{s}$ around mid-move, but the speed it can reach there, $1.13\,\mathrm{rad/s}$, is only just short of the reference's $1.17$, so the error grows only to $19.5\,\mathrm{mrad}$. At $n=300$ it rides the limit for $0.49\,\mathrm{s}$ and the joint tops out at $0.77\,\mathrm{rad/s}$ — the voltage line's speed at the $0.93\,\mathrm A$ it draws at mid-move, $(24-0.93)/(0.1\cdot300)$ — so the arm trails by $118\,\mathrm{mrad}$ and finishes $0.32\,\mathrm{s}$ late.
...
An actuator claim in a paper is usually one number with the condition that makes it true left off — the duration, the pose, the supply voltage, the direction of power flow — so reading it means asking for that condition, row by row.
...
2. Another motor's datasheet gives a speed constant of $191\,\mathrm{rpm/V}$. What is its torque constant in $\mathrm{N{\cdot}m/A}$, and why is no second measurement needed?
...
> 2. Invert and convert units: $191\,\mathrm{rpm/V}$ is $191\cdot2\pi/60=20.0\,\mathrm{rad/s}$ per volt, so $k_e=1/20.0=0.050\,\mathrm{V{\cdot}s/rad}$, and in SI the torque constant is the same number, $0.050\,\mathrm{N{\cdot}m/A}$ — half this page's motor. No second measurement is needed because $k_t=k_e$ is a statement of power conservation, $e\,i=\tau_m\omega_m$, not a property of one motor.
...
1. **Draw.** The picture above, for the $n=50$ drive: its current limit, its voltage line with both ends, its thermal line, the hold point H with its current, and the lift's loop, whose peak demand at the peak acceleration of $4.51\,\mathrm{rad/s^2}$ is $(M_{11}+\eta n^2J_m)\ddot\theta+g_1$, with $g_1=21.94\,\mathrm{N{\cdot}m}$ at that instant. Say which boundary binds the hold and which the lift, and how long the hold lasts from ambient.
2. **Derive.** (a) P2 at the pose where the shoulder's gravity torque is largest, $\theta=(-26.57^\circ,\,90^\circ)$, with $g_1=9.81\sqrt5=21.94\,\mathrm{N{\cdot}m}$ (differentiate $2\cos\theta_1-\sin\theta_1$ to find it), on the $n=100$ drive: the motor torque, current, voltage, heat and steady rise. Does it hold indefinitely? (b) The smallest whole gear ratio that holds that pose continuously. (c) At $n=160$, the mass matrix at $\theta_2=90^\circ$ with both drives, and its coupling $M_{12}/\sqrt{M_{11}M_{22}}$. (d) §4 derived the ratio $n^\ast$ that minimizes the motor torque per unit of joint acceleration, gravity aside, for the shoulder at $\theta_2=90^\circ$. Evaluate it for the elbow, $M_{22}=1\,\mathrm{kg{\cdot}m^2}$ with the shoulder held, and for the shoulder with the arm straight ($\theta_2=0^\circ$, $M_{11}=5$) and folded ($\theta_2=180^\circ$, $M_{11}=1$). Can one gear ratio match every pose, and what does §4's $n^\ast=193.6$ cost at the straight and the folded pose, against each pose's own optimum? (e) The mechanical time constant at $n=200$, and its ratio to $\tau_e$.
...
> - **Check the scaling**: the current limit rises as $n$, the no-load speed falls as $1/n$ and the voltage line's slope steepens as $n^2$ (§3), so halving the ratio must make the region half as tall and twice as wide.

> [!tip]- Solutions
> 1. Current limit $\eta nk_tI_{\max}=0.8\cdot50\cdot0.1\cdot10=40\,\mathrm{N{\cdot}m}$. Voltage line $\tau_j=(0.8\cdot50\cdot0.1/1)(24-0.1\cdot50\,\dot\theta)=96-20\,\dot\theta$, from the corner at $(96-40)/20=2.8\,\mathrm{rad/s}$ down to the no-load speed $4.8\,\mathrm{rad/s}$. Thermal line $0.8\cdot50\cdot0.1\cdot3.162=12.65\,\mathrm{N{\cdot}m}$, meeting the voltage line at $4.17\,\mathrm{rad/s}$. The region is half as tall and twice as wide as at $n=100$. H at $19.62\,\mathrm{N{\cdot}m}$ now needs $19.62/(0.8\cdot50\cdot0.1)=4.905\,\mathrm{A}$ and lies *above* the thermal line, so the hold is heat-limited: from ambient it lasts $-60\ln(1-100/240.6)=32.2\,\mathrm{s}$. The lift fits: its peak demand is $(3+0.8\cdot50^2\cdot10^{-4})\cdot4.51+21.94=3.2\cdot4.51+21.94=36.4\,\mathrm{N{\cdot}m}$, under the $40\,\mathrm{N{\cdot}m}$ current limit, and at mid-move it asks $21.5\,\mathrm{N{\cdot}m}$ at $1.17\,\mathrm{rad/s}$, where the voltage line offers $96-20\cdot1.17=72.6$, capped at $40$. At $n=50$ heat binds, not voltage: the ratio that makes the move easy makes the all-day hold impossible — the low end of §8's window.
> 2. (a) $\tau_m=21.94/(0.8\cdot100)=0.2742\,\mathrm{N{\cdot}m}$, $i=2.742\,\mathrm{A}$, $V=Ri=2.74\,\mathrm{V}$, $P=i^2R=7.52\,\mathrm{W}$, $\Delta T=R_{th}P=75.2\,\mathrm{K}$. Under $100\,\mathrm{K}$, so yes, indefinitely, at $75\%$ of the heat budget. (b) $n\ge21.94/(0.8\cdot0.1\cdot3.162)=86.7$, so $n=87$. (c) $n^2J_m=2.56$ and $nJ_m=0.016$, so $M=\begin{pmatrix}5.5601&1.016\\1.016&3.56\end{pmatrix}$ and the coupling is $1.016/\sqrt{5.5601\cdot3.56}=0.228$, against $0.577$ bare. (d) The optimum is $n^\ast=\sqrt{M/(\eta J_m)}$ for whatever inertia $M$ the joint drives, so for the elbow $n^\ast=\sqrt{1/(0.8\cdot10^{-4})}=111.8$ — the frozen $n=100$ is within $0.6\%$ of its minimum — for the straight arm $\sqrt{5/(0.8\cdot10^{-4})}=250$, and folded $111.8$ again. No single ratio matches every pose, because $M_{11}=3+2\cos\theta_2$ moves with the elbow. The minimum is shallow, though: at $n=193.6$ the motor torque per unit acceleration, $nJ_m+M/(\eta n)$, is $0.0516$ against the straight arm's optimum $2\sqrt{J_mM/\eta}=0.0500\,\mathrm{N{\cdot}m}$ per rad/s², $3.3\%$ more, and $0.0258$ against the folded arm's $0.0224$, $15.5\%$ more — which is why §4 leaves the choice of ratio to gravity and speed, not to inertia matching. (e) $J=10^{-4}+3/(0.8\cdot4\cdot10^4)=1.9375\times10^{-4}\,\mathrm{kg{\cdot}m^2}$, so $\tau_{\text{mech}}=JR/(k_tk_e)=19.4\,\mathrm{ms}$, $19$ times $\tau_e$ — still clear of the ten-times floor.
...
*P2(카탈로그의 평면 2링크 팔, [[02-foundations/lab-plants|0.6]])의 $M$과 $g$는 [[02-foundations/manipulator-kinematics-dynamics|기초 10. 매니퓰레이터 기구학·동역학]] 위에, 기어·일률·회로로 본 모터는 물리 층의 [[02-foundations/basic-mechanics|0.6.1 기초 역학]]과 [[02-foundations/basic-circuits-electronics|0.6.2 기초 회로와 전자]] 위에, PD·대역폭·anti-windup은 [[04-robotics/control-theory-ce397|5. 제어 이론]] 위에 선다. 액추에이터를 embodiment의 일부로 꼽기만 하고 열어 보지는 않는 [[04-robotics/robot-systems-deployment|10. 로봇 시스템 §2]] 옆에 놓인다. 다시 쓰는 장치는 **P2**. 이제 관절마다 모터와 기어박스가 달려 있다.*

> [!note] 왜 배우는가 · Why this matters
> 이 페이지는 [[07-research-program/index|7. 연구 프로그램 §5]]의 로봇 스택 옆 시스템 열에, [[02-foundations/basic-mechanics|0.6.1]]과 [[02-foundations/basic-circuits-electronics|0.6.2]]의 물리 층 바로 위에 선다. 조작 층이 움직일 때 거치는 구동계를 여는 페이지이고, "*저 패널을 프레임에 설치해*"에서는 *부재를 옮기는* 단계를 받친다 — 계획기나 제어기가 관절에 요구하는 모든 토크는 권선에서 전류·전압·열로 나와야 한다. 그리고 §7처럼 모터 전류를 관절 토크로 읽을 때는 *접촉을 감지하는* 단계도 받친다. 이 페이지의 칩은 [[physical-ai-map|피지컬 AI 지도]]의 시스템 열에 있다. 이것이 없으면 관절을 시연으로 판단하게 된다. 카탈로그의 평면 2링크 팔 **P2**([[02-foundations/lab-plants|0.6]])를 카탈로그 자세에 붙잡는 데 $n=100$ 구동계는 $2.45\,\mathrm A$와 권선 상승 $60.1\,\mathrm K$를 들이지만, 같은 유지를 $n=50$ 구동계로 하면 10초 시연은 통과하고 $32.2\,\mathrm s$에 열 한계에 닿으며(§6), $n=300$ 구동계에서 전류를 토크로 읽으면 들어올리기의 최대 토크를 $93\,\%$ 부풀린다(§7). [[04-robotics/force-compliance-control|13. 힘·컴플라이언스 제어]]는 §2(토크 기반 임피던스는 역구동 가능한 관절을 원한다)와 §5(충돌 질량 속 회전자의 $n^2J_m$)에서, [[04-robotics/teleoperation-demonstration|12. 원격조작과 시연 §4.5]]는 반사 관성에서 이 페이지에 기댄다. 학위논문 경로([[07-research-program/index|7. 연구 프로그램 §8]])에서는 블록 2, 로보틱스 90–94회차이고, 12와 13이 블록 3에서 뒤따른다. 이 페이지를 마치면 어떤 관절 토크든 암페어·볼트·와트·켈빈으로 바꾸고, 유지와 동작을 구동계의 토크–속도 영역 위에 놓고, 어떤 팔이나 논문에든 그 토크가 연속인지 최대인지, 그 전류를 힘으로 읽을 수 있는지 물을 수 있다.

> [!note] 처음이라면 · First pass
> 60–90분짜리 회차 다섯 번쯤, 로보틱스 90–94회차다. **첫 회차:** 이 페이지의 대상, 그림, '대상으로 한 번 끝까지'를 손으로 한다. 시작에 그린 구동 사슬을 따라, 관절 토크 하나가 암페어·볼트·와트·켈빈으로 바뀌고 눈에 보이지 않는 회전자 하나가 P2 질량 행렬의 원소 하나를 바꾼다. 끝으로 답을 가리고 2–5단계를 $n=80$에서 다시 한다($3.07\,\mathrm A$, $94\,\mathrm K$: 여전히 버틴다). **둘째 회차:** §1–§3, 곧 모터의 두 방정식, 기어박스, 토크–속도 선. **셋째 회차:** §4–§6, 곧 반사 관성, 두 시정수, 열. §6의 구리 메모는 두 번째 읽기다. **넷째 회차:** §8의 랩과 과제 3. **다섯째 회차:** 랩의 들어올리기를 모터 전류 쪽에서 다시 읽는 §7의 역구동성(충격 완화 메모는 두 번째 읽기), 그다음 §9, 스스로 점검, 과제 1–2.

### 이 페이지의 대상 · Running object

대상은 [[02-foundations/lab-plants|0.6 Lab Plants]]의 **P2**. 연직면에 세우고 $g=9.81\,\mathrm{m/s^2}$를 $-y$ 방향으로 두며, 고정 자세 $\theta=(0^\circ,90^\circ)$에 놓는다. 엘보는 $(1,0)$, 전완은 똑바로 위, 말단은 $(1,1)\,\mathrm{m}$. 이 페이지 전체를 끌고 가는 숫자는 둘이고, 둘 다 [[02-foundations/manipulator-kinematics-dynamics|기초 10 §3과 §5]]에서 유도했다. 어깨의 질량 행렬 원소 $M_{11}=3\,\mathrm{kg{\cdot}m^2}$와 어깨의 중력 토크 $g_1=19.62\,\mathrm{N{\cdot}m}$다. 이 자세에서 엘보의 중력 토크는 $0$인데, 전완 질량이 엘보 바로 위에 있기 때문이다.

이제 관절마다 **구동계**(drive)가 붙는다. DC 모터, 그 모터에 전력을 주는 증폭기(PWM 브리지, [[02-foundations/basic-circuits-electronics|0.6.2 §6]]), 그리고 모터와 링크 사이의 기어박스다. 카탈로그에는 구동계가 없으므로 이 페이지가 하나를 완전히 정의하고 고정한다. 값은 **예시용**: 작은 팔 관절에 맞는 크기로 이 페이지가 고른 것이고, 어떤 제품의 데이터시트에서 가져온 것이 아니다.
...
모델링 선택 셋을 한 번만 적어 둔다. **두 구동계는 똑같고**, 관절마다 하나씩이다. **구동계가 팔에 더하는 것은 회전자의 자전 관성뿐이다.** 구동계의 질량은 카탈로그의 점질량 안에 들어 있다고 본다. 그래서 베이스에 볼트로 고정된 어깨 구동계는 질량을 더하지 않고, 엘보 구동계의 질량은 엘보의 $1\,\mathrm{kg}$에 포함된다. **전기 모델은 브러시 DC 모터의 것이다.** 전류 제어기 아래의 브러시리스 모터도 같은 꼴의 식을 따르고, *Modern Robotics* §8.9.1도 같은 단순화를 한다. 랩(§8)에서는 엘보 구동계가 엘보를 $90^\circ$에 붙잡고 어깨만 움직이므로, 어깨는 정확히 $M_{11}$과 $g_1(\theta_1)=9.81\,(2\cos\theta_1-\sin\theta_1)\,\mathrm{N{\cdot}m}$를 본다. 기초 10의 $g_1$에서 $\theta_2$를 $90^\circ$로 고정한 것이다.
...
<svg viewBox="0 0 560 284" style="max-width:100%;height:auto" role="img" aria-label="감속비 100에서 P2 어깨 구동계의 관절 토크 대 관절 속도: 전류 한계 80 N m, 1.4에서 2.4 rad/s까지의 전압선, 25.3 N m의 연속(열) 한계선, 19.62 N m의 유지점 H, 그리고 21.9 N m의 시작점에서 39.1 N m까지 올랐다가 H로 돌아오는 들어올리기의 요구 고리">
  <path d="M70 240 L70 80 L312.3 80 L485.4 240 Z" fill="currentColor" fill-opacity="0.07"/>
  <g stroke="currentColor" stroke-width="1" opacity="0.55" fill="none">
    <line x1="70" y1="240" x2="530" y2="240"/><line x1="70" y1="240" x2="70" y2="48"/>
  </g>
  ...
  <circle cx="70" cy="196.2" r="3.2" fill="none" stroke="currentColor" stroke-width="1.3"/>
  <g font-size="11" fill="currentColor">
    <text x="540" y="22" text-anchor="end" xml:space="preserve">P2 어깨 구동계 · n = 100 · V<tspan dy="3" font-size="10">s</tspan><tspan dy="-3"> = 24 V</tspan></text>
    <text x="12" y="40" xml:space="preserve">관절 토크 η·n·k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3">·i (N·m)</tspan></text>
    ...
    <text x="290" y="206" font-size="10.5">연속, 25.3 N·m</text>
    <text x="100" y="151">들어올리기: 0.8 s 동안 0.5 rad</text>
    <text x="84" y="216">← H: 유지, 19.62 N·m, 2.45 A</text>
    <text x="64" y="190" text-anchor="end" font-size="10.5">시작 21.9</text>
  </g>
</svg>


$n=100$ 구동계를 단 P2 어깨의 운전 영역을 관절 단위로 그렸다. 전류 한계는 $80\,\mathrm{N{\cdot}m}$의 수평선, 전압선 $\tau=192-80\,\dot\theta$는 $1.4\,\mathrm{rad/s}$의 모서리에서 무부하 속도 $2.4\,\mathrm{rad/s}$까지 내려가고, 점선의 열 한계선은 연속 토크 $25.3\,\mathrm{N{\cdot}m}$에 있어 그 위에는 몇 초 동안만 머물 수 있다. 유지점 H($19.62\,\mathrm{N{\cdot}m}$, $2.45\,\mathrm{A}$)는 열 한계선 아래에 있고, §8의 들어올리기 — 어깨를 $0.8\,\mathrm{s}$에 $0.5\,\mathrm{rad}$ 올리는 동작 — 는 $-0.5\,\mathrm{rad}$에서 중력 $21.9\,\mathrm{N{\cdot}m}$로 정지한 채 출발해, 가속하는 동안 $39.1\,\mathrm{N{\cdot}m}$까지 오르고 중간에서 $1.17\,\mathrm{rad/s}$, $21.5\,\mathrm{N{\cdot}m}$를 지나 H에서 끝나는 고리를 그리며, 영역을 벗어나지 않는다.

### 대상으로 한 번 끝까지 · Worked case

$n=100$ 구동계로 P2를 고정 자세에 붙잡고, 숫자 하나 — 어깨의 중력 토크 — 를 권선까지 따라 내려갔다가 열이 되어 나오는 데까지 따라간다. 그다음 구동계가 질량 행렬에 무슨 일을 했는지 본다. 여섯 단계를, 여기 그린 사슬을 따라 밟는다.

<svg viewBox="0 0 560 292" style="max-width:100%;height:auto" role="img" aria-label="P2 어깨 구동계를 다섯 단의 사슬로: 공급·증폭기, 권선, 회전자, 기어박스, 링크. 유지에서는 2.45 V가 2.45 A를 흘리고, 그것이 회전자에서 0.2453 N·m가 되어 기어박스에서 80배로 링크의 19.62 N·m가 되며, 열 6.01 W가 10 K/W를 지나 권선을 60.1 K 올린다. 들어올리기의 0.40 s에는 공급에서 42.5 W가 나가, 8.0 W가 권선을 데우고 0.1 W가 인덕턴스에서 돌아오며, 34.6 W가 역기전력을 건너, 0.5 W는 회전자와 베어링에, 34.1 W가 기어박스에 들어가 6.8 W를 잃고 27.3 W가 링크에 닿는다.">
  <defs><marker id="dcAk" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor"/></marker></defs>
  <g fill="currentColor">
    <text x="12" y="18" font-size="11.5" font-weight="600" fill-opacity="0.85">구동 사슬: 유지(1–5단계)와 들어올리기의 일률(t = 0.40 s, §8)</text>
    <path d="M 10.0 38 L 10.0 32 L 216.0 32 L 216.0 38" fill="none" stroke="currentColor" stroke-width="0.8" stroke-opacity="0.55"/>
    <text x="113.0" y="29" font-size="10" text-anchor="middle" fill-opacity="0.75" font-style="italic">전기</text>
    <path d="M 230.0 38 L 230.0 32 L 546.0 32 L 546.0 38" fill="none" stroke="currentColor" stroke-width="0.8" stroke-opacity="0.55"/>
    <text x="388.0" y="29" font-size="10" text-anchor="middle" fill-opacity="0.75" font-style="italic">기계</text>
    <rect x="10.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="58" y="59" font-size="11" font-weight="600" text-anchor="middle">공급·증폭기</text>
    <text x="58" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">V<tspan dy="3" font-size="10">s</tspan><tspan dy="-3"> = 24 V</tspan></text>
    <text x="58" y="88" font-size="10.5" text-anchor="middle" xml:space="preserve">I<tspan dy="3" font-size="10">max</tspan><tspan dy="-3"> = 10 A</tspan></text>
    <rect x="120.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="168" y="59" font-size="11" font-weight="600" text-anchor="middle">권선</text>
    <text x="168" y="74" font-size="10.5" text-anchor="middle">R = 1.0 Ω</text>
    <text x="168" y="88" font-size="10.5" text-anchor="middle">L = 1.0 mH</text>
    <rect x="230.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="278" y="59" font-size="11" font-weight="600" text-anchor="middle">회전자</text>
    <text x="278" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">J<tspan dy="3" font-size="10">m</tspan><tspan dy="-3"> = 10⁻⁴ kg·m²</tspan></text>
    <text x="278" y="88" font-size="10.5" text-anchor="middle" xml:space="preserve">k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3"> = 0.10 N·m/A</tspan></text>
    <rect x="340.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="388" y="59" font-size="11" font-weight="600" text-anchor="middle">기어박스</text>
    <text x="388" y="74" font-size="10.5" text-anchor="middle">n = 100</text>
    <text x="388" y="88" font-size="10.5" text-anchor="middle">η = 0.80</text>
    <rect x="450.0" y="44" width="96" height="50" rx="4" fill="currentColor" fill-opacity="0.08" stroke="currentColor" stroke-width="1" stroke-opacity="0.7"/>
    <text x="498" y="59" font-size="11" font-weight="600" text-anchor="middle">링크: P2 어깨</text>
    <text x="498" y="74" font-size="10.5" text-anchor="middle" xml:space="preserve">M<tspan dy="3" font-size="10">11</tspan><tspan dy="-3"> = 3 kg·m²</tspan></text>
    <text x="498" y="88" font-size="10.5" text-anchor="middle">(0°, 90°)에서</text>
    <line x1="107.0" y1="69.0" x2="119.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcAk)"/>
    <line x1="217.0" y1="69.0" x2="229.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcAk)"/>
    <line x1="327.0" y1="69.0" x2="339.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcAk)"/>
    <line x1="437.0" y1="69.0" x2="449.0" y2="69.0" stroke="currentColor" stroke-width="1.3" marker-end="url(#dcAk)"/>
    <path d="M 58 96 L 58 104 M 58 119 L 58 164 M 58 178 L 58 188 M 58 203 L 58 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 168 96 L 168 104 M 168 119 L 168 164 M 168 178 L 168 188 M 168 203 L 168 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 278 96 L 278 104 M 278 119 L 278 164 M 278 178 L 278 188 M 278 203 L 278 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 388 96 L 388 104 M 388 119 L 388 164 M 388 178 L 388 188 M 388 203 L 388 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <path d="M 498 96 L 498 104 M 498 119 L 498 164 M 498 178 L 498 188 M 498 203 L 498 280" fill="none" stroke="currentColor" stroke-width="0.7" stroke-opacity="0.3" stroke-dasharray="2 3"/>
    <text x="12" y="114" font-size="10.5" font-style="italic" fill-opacity="0.85">유지: 19.62 N·m를 권선까지 따라가 열로 나오게 한다</text>
    <text x="62" y="131" font-size="11" font-weight="600">2.45 V</text>
    <text x="62" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">24 V 중</text>
    <text x="172" y="131" font-size="11" font-weight="600">2.45 A</text>
    <text x="172" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">열 6.01 W</text>
    <text x="282" y="131" font-size="11" font-weight="600">0.2453 N·m</text>
    <text x="282" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">= k<tspan dy="3" font-size="10">t</tspan><tspan dy="-3"> i</tspan></text>
    <text x="392" y="131" font-size="11" font-weight="600">× ηn = 80</text>
    <text x="392" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">× 100이 아님</text>
    <text x="502" y="131" font-size="11" font-weight="600">19.62 N·m</text>
    <text x="502" y="145" font-size="10.5" fill-opacity="0.85" xml:space="preserve">중력, g₁</text>
    <line x1="172" y1="150" x2="172" y2="164" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcAk)"/>
    <text x="178" y="174" font-size="10.5" xml:space="preserve">열: R<tspan dy="3" font-size="10">th</tspan><tspan dy="-3"> = 10 K/W</tspan>이므로 ΔT = 60.1 K</text>
    <text x="12" y="198" font-size="10.5" font-style="italic" fill-opacity="0.85">들어올리기의 t = 0.40 s: 1.226 rad/s, 2.824 A, 15.05 V — 와트 단위로</text>
    <circle cx="58" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="168" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="278" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="388" cy="224" r="3.5" fill="currentColor"/>
    <circle cx="498" cy="224" r="3.5" fill="currentColor"/>
    <line x1="64" y1="224" x2="162" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcAk)"/>
    <text x="113.0" y="218" font-size="11" font-weight="600" text-anchor="middle">42.5 W</text>
    <line x1="174" y1="224" x2="272" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcAk)"/>
    <text x="223.0" y="218" font-size="11" font-weight="600" text-anchor="middle">34.6 W</text>
    <line x1="284" y1="224" x2="382" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcAk)"/>
    <text x="333.0" y="218" font-size="11" font-weight="600" text-anchor="middle">34.1 W</text>
    <line x1="394" y1="224" x2="492" y2="224" stroke="currentColor" stroke-width="2" marker-end="url(#dcAk)"/>
    <text x="443.0" y="218" font-size="11" font-weight="600" text-anchor="middle">27.3 W</text>
    <line x1="168" y1="229" x2="168" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcAk)"/>
    <text x="172" y="258" font-size="10.5">열 8.0 W,</text>
    <text x="172" y="271" font-size="10.5" fill-opacity="0.85">L에서 0.1 W 돌아옴</text>
    <line x1="278" y1="229" x2="278" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcAk)"/>
    <text x="282" y="258" font-size="10.5">회전자·베어링</text>
    <text x="282" y="271" font-size="10.5" fill-opacity="0.85">0.5 W</text>
    <line x1="388" y1="229" x2="388" y2="246" stroke="currentColor" stroke-width="1.1" marker-end="url(#dcAk)"/>
    <text x="392" y="258" font-size="10.5">손실 6.8 W</text>
    <text x="392" y="271" font-size="10.5" fill-opacity="0.85">(1 − η)</text>
    <text x="458" y="246" font-size="10.5">팔을 들어 올리고</text>
    <text x="458" y="259" font-size="10.5">빠르게 한다: 64 %</text>
  </g>
</svg>

P2 어깨의 구동계를 공급에서 링크까지 사슬 하나로 그렸다. 전기 단과 기계 단이 있고, 열이 셋째 영역이다. 위는 1–5단계의 유지다. 링크의 $19.62\,\mathrm{N{\cdot}m}$는 회전자의 $0.2453\,\mathrm{N{\cdot}m}$의 $\eta n=80$배이고, 그 토크에 $24\,\mathrm V$ 공급 가운데 $2.45\,\mathrm V$로 $2.45\,\mathrm A$가 들며, 열 $6.01\,\mathrm W$가 $R_{th}$를 거쳐 권선을 주변보다 $60.1\,\mathrm K$ 높은 곳에 자리 잡게 한다. 아래는 같은 사슬이 §8 들어올리기의 $t=0.40\,\mathrm s$에 나르는 일률이다. 공급에서 $42.5\,\mathrm W$, 열 $8.0\,\mathrm W$와 인덕턴스에서 돌아오는 $0.1\,\mathrm W$, 역기전력을 건너는 $34.6\,\mathrm W$, 회전자와 베어링의 $0.5\,\mathrm W$, 기어박스로 들어가는 $34.1\,\mathrm W$, 거기서 잃는 $6.8\,\mathrm W$, 그리고 링크에 닿는 $27.3\,\mathrm W$, 곧 공급의 $64\,\%$다.

**1단계 — 관절이 내야 하는 것.** 정지 상태에서 [[02-foundations/manipulator-kinematics-dynamics|기초 10 §5]]의 매니퓰레이터 방정식은 $\tau=g(\theta)$만 남기므로, 어깨는 유지 토크 $g_1=19.62\,\mathrm{N{\cdot}m}$를 내야 하고 엘보는 아무것도 내지 않는다. [[02-foundations/basic-mechanics|0.6.1 §8]]의 정역학이 자유물체도에서 같은 두 숫자를 준다.

**2단계 — 기어박스를 거쳐.** 모터가 구동할 때 기어박스는 토크를 $\eta n$배 한다(§2. 손실 없는 지렛대와 기어의 맞바꿈은 [[02-foundations/basic-mechanics|0.6.1 §9]]). 그래서 모터가 만들어야 하는 토크는
...
**4단계 — 전압.** 단자 전압은 키르히호프 전압 법칙([[02-foundations/basic-circuits-electronics|0.6.2 §3]])에 따라 세 곳으로 나뉜다. $V=Ri+L\,di/dt+k_e\omega_m$, 곧 저항의 강하, 인덕턴스의 강하([[02-foundations/basic-circuits-electronics|0.6.2 §4]]), 도는 회전자의 역기전력이다(§1). 아무것도 돌지 않으니 역기전력이 없고, 전류가 일정하니 인덕턴스에 걸리는 전압도 없다. 그래서 $24\,\mathrm{V}$ 공급 중 $V=Ri=2.45\,\mathrm{V}$만 남는다.

**5단계 — 열.** 공급은 $Vi=6.01\,\mathrm{W}$를 내고 팔은 아무 일도 하지 않으므로 — 가만히 붙잡는 데는 기계적 일률이 들지 않는다([[02-foundations/basic-mechanics|0.6.1 §6]]) — 그 와트가 전부 권선 저항의 열이 된다. $P=i^2R=2.4525^2\cdot1.0=6.01\,\mathrm{W}$([[02-foundations/basic-circuits-electronics|0.6.2 §2]]). 열저항을 거쳐 권선은
...
**6단계 — 구동계가 질량 행렬에 한 일.** 어깨 회전자는 링크 1이 한 바퀴 돌 때 $n=100$바퀴 돌므로, 운동 에너지가 $\tfrac12J_m(n\dot\theta_1)^2$이고 $M_{11}$에 $n^2J_m=10^4\cdot10^{-4}=1.0\,\mathrm{kg{\cdot}m^2}$를 더한다(§4). 똑같은 엘보 구동계는 $M_{22}$에 같은 양을, 비대각 원소에 $nJ_m=0.01$을 더한다.

$$M=\begin{pmatrix}3&1\\1&1\end{pmatrix}\ \longrightarrow\ \begin{pmatrix}4.0001&1.01\\1.01&2\end{pmatrix}\ \mathrm{kg{\cdot}m^2}$$

여기서 $0.0001$은 링크 1에 올라타 함께 도는 엘보 회전자다. 모든 원소는 관절 속도 제곱당 운동 에너지이므로([[04-robotics/modern-robotics/ch08-dynamics|MR 8장 §2]]) 기어박스의 손실은 여기에 나타나지 않는다. 어깨 관성의 $1/30{,}000$밖에 안 되는 회전자가 그 3분의 1이 되었고, 엘보에서는 구동계가 팔을 두 배로 만들었다. 모터가 손실 있는 기어박스를 거쳐 밀어야 하는 관성으로 세면 — 여전히 관절 단위로 — 어깨는 더 무겁다.

$$J_{eq}=n^2J_m+\frac{M_{11}}{\eta}=1.0+\frac{3}{0.8}=1.0+3.75=4.75\ \mathrm{kg{\cdot}m^2}$$

기어박스를 지나는 링크의 몫만 $\eta$의 통행료를 내기 때문이다(유도는 §2). 질량 행렬의 $4.0001$보다 큰 것은 둘이 서로 다른 질문에 답하기 때문이다. 질량 행렬은 저장된 에너지를 세고, $J_{eq}$는 모터가 밀어야 하는 것이다.

과제는 이 여섯 단계를 중력이 가장 큰 자세와 다른 감속비에서 되풀이한다.

### 1. 방정식 둘, 모터 하나

'대상으로 한 번 끝까지'의 $19.62\,\mathrm{N{\cdot}m}$를 암페어와 볼트로 바꾸려면 모터의 두 방정식이 필요하다. DC 모터는 전기 회로 하나와 회전체 하나이고, 둘을 상수 두 개가 잇는다. [[02-foundations/basic-circuits-electronics|0.6.2 §7]]이 회로 소자로서의 모터를 다뤘고, 이 절은 회전자와 두 상수를 더한다. 회로는 키르히호프 전압 법칙([[02-foundations/basic-circuits-electronics|0.6.2 §3]])으로 쓴다. 증폭기가 단자에 건 전압은 권선의 저항, 인덕턴스를 지나는 전류의 변화, 그리고 도는 회전자가 유도하는 전압에 나뉘어 쓰인다.
...
> **권선의 정의.** **권선**(winding)은 모터의 전류를 나르는 구리 코일이고, 전기적으로는 *직렬 저항–인덕터 쌍*이다. 모터의 집중 파라미터 둘이지 부하나 제어기의 성질이 아니다. 정의 조건은 파라미터마다 하나씩, 둘이다. **저항** $R$은 전류를 $i^2R$의 비율로 열로 바꾸고, 멈춘 모터가 끌어가는 전류 $V/R$을 정한다. **인덕턴스** $L$은 [[02-foundations/basic-circuits-electronics|0.6.2 §4]]의 인덕터로서 에너지 $\tfrac12Li^2$을 저장하고 전류의 변화에 맞서므로, 전압이 뛰어도 전류는 뛰지 못한다. 그 걸리는 시간이 §5의 전기적 시정수다.
>
> $$V_R=R\,i,\qquad V_L=L\,\frac{di}{dt},\qquad P_{\text{heat}}=i^2R$$
>
> $V_R$과 $V_L$은 두 소자에 걸리는 전압 강하, $i$는 전류, $P_{\text{heat}}$는 소산되는 일률이다. 그러니 쌍 가운데 에너지를 쓰는 것은 $R$뿐이다. 인덕터는 저장한 것을 돌려주기 때문이다.
> - **예**: 고정한 구동계. $R=1.0\,\Omega$이므로 '대상으로 한 번 끝까지'의 $2.4525\,\mathrm{A}$는 $6.01\,\mathrm{W}$의 열이 들고, $L=1.0\,\mathrm{mH}$이므로 전류를 $1\,\mathrm{ms}$에 $1\,\mathrm{A}$ 바꾸려면 $1\,\mathrm{V}$가 든다.
...
> - **예**: 고정한 구동계, $k_t=k_e=0.10$. '대상으로 한 번 끝까지'의 $2.4525\,\mathrm{A}$는 $0.2453\,\mathrm{N{\cdot}m}$를 만든다. 들어올리기의 중간 속도, 관절에서 $1.17\,\mathrm{rad/s}$이니 모터에서 $117\,\mathrm{rad/s}$일 때 권선에는 $11.7\,\mathrm{V}$의 역기전력이 생긴다.
...
$100{:}1$ 기어박스는 토크를 100배 하지 않는다. 실제 기어박스는 일률에서 제 몫을 떼어 가므로 $\eta n=80$배 하고, 모터는 링크를 감속비를 거쳐서만 느낀다. 토크는 $\eta n$으로, 관성은 $\eta n^2$으로 나뉘어서다. 이 절이 둘을 유도한다. 지렛대와 기어는 힘을 거리와 맞바꾸고, 반사 관성 $n^2J_m$은 회전자의 운동 에너지에서 나온다. [[02-foundations/basic-mechanics|0.6.1 기초 역학 §9]]가 둘을 손실 없는 이상 조건에서 P2의 기어박스 위에 계산한다. 이 절은 기어박스의 효율과 모터가 보는 부하를 더한다.
...
> - **예**: '대상으로 한 번 끝까지'. 관절에 $19.62\,\mathrm{N{\cdot}m}$가 필요하므로 모터 쪽에는 손실 없는 $0.1962$ 대신 $19.62/(0.8\cdot100)=0.2453\,\mathrm{N{\cdot}m}$가 필요하다. 전류는 4분의 1 더, 열은 전류의 제곱으로 가므로 $1/\eta^2=1.5625$배다.
...
이제 부하를 끝까지 옮겨 보자. 링크 쪽에서 기어박스는 관절이 요구하는 것, $\tau_{\text{load}}$를 내야 한다. 엘보를 붙잡은 P2의 어깨라면 $\tau_{\text{load}}=M_{11}\ddot\theta_1+g_1(\theta_1)$다. 모터가 구동할 때 $\tau_{\text{load}}=\eta n\,\tau_{in}$이므로 $\tau_{in}=\tau_{\text{load}}/(n\eta)$이고, 이것을 §1의 회전자 식에 넣으면 구동계의 기계 방정식이 나온다.
...
이다. 그러니 모터가 밀어야 하는 관절의 관성은 $J_{eq}=n^2J_m+M_{11}/\eta$이고, $n=100$ 구동계에서는 $1.0+3.75=4.75\,\mathrm{kg{\cdot}m^2}$다. 회전자는 §4의 에너지 값으로 들어오고, 링크와 그 중력은 $\eta$로 나뉘어 들어온다. 기어박스를 지나는 것만 통행료를 내기 때문이다. 우변은 *이상 기어* 단위로 쓰였다 — $nk_ti$는 손실 없는 기어박스가 전달할 토크다 — 그리고 랩의 제어기도 같은 단위로 토크를 요구한다. $\dot\theta_1=\ddot\theta_1=0$으로 두면 $i=g_1/(\eta nk_t)$, '대상으로 한 번 끝까지'의 2·3단계가 한 줄로 나오고, 랩의 `acc = ...` 줄은 이 식을 $\ddot\theta_1$에 대해 푼 것이다.

이 식은 일률이 모터에서 링크로 흐른다고 가정한다. §8의 들어올리기 내내 정말 그런지는 아래 메모가 확인한다.

> [!note]- 더 깊이 · Deeper
> **적용 영역 조건 하나.** 이것은 모터가 부하를 *구동하는* 식이다. §8의 들어올리기에서 링크 토크 $M_{11}\ddot\theta_1+g_1$은 팔이 올라가는 동안 양수로 남는다 — 모든 행을 통틀어 가장 작은 값이 $n=30$의 $0.86\,\mathrm{N{\cdot}m}$다 — 그래서 올라가는 내내 일률은 모터에서 링크로 흐르고, 이것이 식이 기술하는 방향이다. 거꾸로 흐르는 것은 각 행이 동작 끝에서 조금 넘어섰다가 되돌아오는 구간뿐이고, 거기서는 식이 손실을 엉뚱한 쪽에 둔다. 그것이 무엇을 바꾸는지는 §8이 말한다.
...
로 읽힌다. 기어박스가 모터의 토크를 $\eta n$배 하고 속도를 $n$으로 나누기 때문이다. $n$과 함께 세 가지가 움직이는데, 같은 속도로 움직이지 않는다. 전류 한계 $\eta nk_tI_{\max}$는 $n$에 비례해 오르고, 무부하 속도 $V_s/(k_en)$는 $1/n$로 떨어지고, 전압선의 기울기 $\eta n^2k_tk_e/R$는 $n^2$로 가팔라진다. $n=100$ 구동계에서 그 값은 $80\,\mathrm{N{\cdot}m}$, $2.4\,\mathrm{rad/s}$, rad/s당 $80\,\mathrm{N{\cdot}m}$이고, 맨 위 그림의 선 $\tau_j=192-80\,\dot\theta$다. 감속비를 두 배로 하면 영역은 높이가 두 배, 폭이 절반이 된다. $n=200$에서 $160\,\mathrm{N{\cdot}m}$, $1.2\,\mathrm{rad/s}$. [[04-robotics/modern-robotics/ch09-trajectory-generation|MR 9장]]의 과제가 P2의 엘보에 더 빠르고 더 약한 구동계를 달고, 고정 사양 $0.8$과 $2$ 대신 $v_{\max}=1.6\,\mathrm{rad/s}$, $a_{\max}=0.5\,\mathrm{rad/s^2}$로 계획할 때 계획기의 말로 적은 것이 바로 이 맞바꿈이다. 감속비를 낮추면 관절의 무부하 속도는 $1/n$에 비례해 오르고 전류 한계 토크는 $n$에 비례해 내려간다 — $n$을 절반으로 하면 하나는 두 배, 다른 하나는 절반이 된다. 계획기의 한계는 이 물리적 한계 위가 아니라 그 안쪽에서 고르므로, 그 사양은 속도를 두 배로 올리면서도 가속도를 절반이 아니라 4분의 1로 줄일 수 있다.
...
회전자는 작고 $n^2$은 크다. '대상으로 한 번 끝까지'의 6단계에서 어깨 관성의 $1/30{,}000$밖에 안 되는 회전자가 그 3분의 1이 되었다. 이 절은 그 반사 관성을 정의하고, 그것이 팔보다 무거워지는 감속비를 찾고, 그것이 팔의 동역학에 무엇을 하는지 말한다.

> **반사 관성의 정의.** 전동 장치 뒤에 있는 모터의 **반사 관성**(reflected inertia)은 *그 회전자가 출력에 기여하는 관성*이다. 회전자와 비만의 성질이고, 부하나 제어기, 구동계에 전원이 들어와 있는지와는 무관하다. 정의 조건 셋. **운동 에너지가 같다.** 출력의 속도로 돌면서 회전자가 제 속도로 도는 만큼의 에너지를 저장하는 관성이다. **비의 제곱으로 자란다.** 회전자는 출력의 $n$배로 돌고 운동 에너지는 속도의 제곱으로 가기 때문이다. 그리고 **전원이 꺼져도 있다.** 제어의 효과가 아니라 움직이는 질량이기 때문이다. 회전 관절에서는 유도가 한 줄이다. 어깨의 고정자는 베이스에 볼트로 고정되어 있고 회전자는 $\omega_m=n\dot\theta_1$로 도므로
>
> $$\tfrac12J_m\,\omega_m^2=\tfrac12J_m\,(n\dot\theta_1)^2=\tfrac12\,\big(n^2J_m\big)\,\dot\theta_1^2\quad\Longrightarrow\quad J_{\text{refl}}=n^2J_m$$
>
> 이고, 이것은 관절 자신의 속도로 도는 관성 $n^2J_m$의 운동 에너지다. $J_m$은 회전자 관성, $\dot\theta_1$은 관절 속도다.
> - **예**: $n=100$인 고정 구동계. 어깨에서 $10^4\cdot10^{-4}=1.0\,\mathrm{kg{\cdot}m^2}$로, 팔 자신의 $M_{11}=3$의 3분의 1이다 — '대상으로 한 번 끝까지'의 6단계.
...
회전자가 $M_{11}$에 더하는 것이 정확히 이것이고, 효율은 들어 있지 않다. 질량 행렬은 운동 에너지이고 마찰은 아무것도 저장하지 않기 때문이다. *Modern Robotics* §8.9.2는 이것을 겉보기 관성(apparent inertia)이라 부르고 같은 방식으로 유도한다. 캡스턴 판은 [[04-robotics/haptics-teleoperation/device-design-kinematics|24.3 §3]]에서 골라 더 읽을 수 있다. 굴착기도 같은 $n^2$을 따른다. [[02-foundations/fluid-power|0.6.3 유체 동력 §7]]의 $600\,\mathrm{kg}$ 팔은 실린더 로드보다 $6$배 빨리 움직이므로, 로드에서는 $6^2\times600=21\,600\,\mathrm{kg}$으로 느껴진다.

엘보 구동계는 더 미묘한데, 고정자가 링크 1에 올라타 있기 때문이다. 회전자의 절대 속도는 $\dot\theta_1+n\dot\theta_2$이고, 이것을 제곱하면 회전자의 에너지가 셋으로 갈라진다.

$$\tfrac12J_m(\dot\theta_1+n\dot\theta_2)^2=\tfrac12J_m\dot\theta_1^2+nJ_m\,\dot\theta_1\dot\theta_2+\tfrac12n^2J_m\dot\theta_2^2$$

그래서 [[04-robotics/modern-robotics/ch08-dynamics|MR 8장 §2]]처럼 이차형식에서 계수를 읽으면, $M_{11}$에 $J_m$, $M_{12}$에 $nJ_m$, $M_{22}$에 $n^2J_m$을 더한다. $4.0001$과 $1.01$이 있는 '대상으로 한 번 끝까지'의 행렬이다.

**왜 지배할 수 있는가.** 회전자와 팔의 겨루기는 비 하나로 정해진다.

$$\frac{n^2J_m}{M_{ii}}=1\quad\Longleftrightarrow\quad n=\sqrt{M_{ii}/J_m}$$

관절 $i$에서 회전자의 반사 관성이 팔 자신의 관성과 같아지는 곳이 거기이기 때문이다. 고정한 구동계에서 어깨는 $n=\sqrt{3/10^{-4}}=173$에서, 엘보는 $n=\sqrt{1/10^{-4}}=100$에서 동등해진다. 그러니 $n=100$ 팔의 엘보는 정확히 동등점에 있고, $n=300$에서 어깨 회전자는 팔의 세 배인 $9\,\mathrm{kg{\cdot}m^2}$다. 세 가지 결과가 따른다.

- **질량 행렬이 더 대각이 된다.** 결합을 $M_{12}/\sqrt{M_{11}M_{22}}$로 재면, 맨 팔의 $0.577$에서 $n=100$의 $0.357$, $n=300$의 $0.094$로 떨어진다. *Modern Robotics* §8.9.2도 같은 관찰을 한다. 감속비가 크면 각 관절은 주로 자기 회전자를 느낀다. [[02-foundations/manipulator-kinematics-dynamics|기초 10 §3]]이 다섯 배로 잰 자세 의존성도 함께 줄어든다. $M_{11}$은 맨 팔에서 접힌 $1$부터 편 $5$까지, $n=100$에서 $2$부터 $6$까지, $n=300$에서 $10$부터 $14$까지 움직여 비가 $1.4$다. 감속비가 큰 팔에서는 고정 이득 관절 제어기가 거의 일정한 관성을 만나고, 대가는 다른 데서 치른다.
- **말단이 무거워지고, 충돌이 세진다.** 말단을 똑바로 위로 미는 힘이 만나는 질량은 $JM^{-1}J^\top$의 $yy$ 원소의 역수이고, [[02-foundations/manipulator-kinematics-dynamics|기초 10 §6]]의 "미는 힘이 느끼는 질량의 역수"라는 읽기다. 맨 팔에서 $2.0\,\mathrm{kg}$, $n=100$ 구동계로 $3.49\,\mathrm{kg}$, $n=300$에서 $11.9\,\mathrm{kg}$. 최대 충돌력은 그 질량의 제곱근으로 가므로, [[04-robotics/force-compliance-control|13. §5]]의 $5\,\mathrm{cm/s}$ 착지는 $22.4\,\mathrm{N}$에서 $n=100$의 $29.5\,\mathrm{N}$, $n=300$의 $54.5\,\mathrm{N}$로 오른다. 몇 밀리초 만에 끝나는 충돌에서는 제어기가 이 관성을 가릴 시간이 없고, 관성은 전원이 꺼져도 있다.
...
> **전기적·기계적 시정수의 정의.** **시정수**(time constant)는 1차 응답이 계단의 $1-e^{-1}=63\%$를 가는 데 걸리는 *시간*이다. 실수 극점 하나의 역수이고, [[02-foundations/basic-circuits-electronics|0.6.2 §4]]가 회로에 대해 정의한 것과 같다. 모터에는 §1의 방정식마다 하나씩 둘이 있고, 각각은 다른 하위계를 비켜 둔 채 정의한다. **전기적 시정수** $\tau_e=L/R$은 회전자를 붙잡은 채 전압 계단을 줬을 때 전류가 자리 잡는 빠르기다. **기계적 시정수** $\tau_{\text{mech}}=JR/(k_tk_e)$는 $L$을 무시하고 전압 계단을 줬을 때 속도가 자리 잡는 빠르기다. 여기서 역기전력은 댐퍼 $k_tk_e/R$로 작용하고, $J$는 모터가 가속하는 *모든 것*을 모터 축으로 환산한 것이다.
>
> $$\tau_e=\frac{L}{R},\qquad \tau_{\text{mech}}=\frac{J\,R}{k_t\,k_e},\qquad J=J_m+\frac{M_{11}}{\eta\,n^2}$$
>
...
> - **왜 중요한가**: $\tau_{\text{mech}}$는 감속비나 부하가 무엇이든 회전자만의 $J_mR/(k_tk_e)=10\,\mathrm{ms}$ 아래로 떨어질 수 없다. 그래서 전류는 속도가 바뀔 수 있는 것보다 적어도 열 배 빨리 자리 잡는다. 그 분리가 아래 캐스케이드의 근거다.
...
영점이 극점 위에 앉으면 루프 이득 $C_i(s)\cdot1/(Ls+R)$이 $K_{pi}/(Ls)$로 줄어들기 때문이고, 역기전력은 적분기가 없애는 느린 외란이다. $\omega_{ci}=2000\,\mathrm{rad/s}$($318\,\mathrm{Hz}$)로 두면 $K_{pi}=L\omega_{ci}=2\,\mathrm{V/A}$, $K_{ii}=R\omega_{ci}=2000\,\mathrm{V/(A{\cdot}s)}$이고, [[04-robotics/control-theory-ce397|5. 제어 이론 §7]]의 조건부 적분 anti-windup을 달고 $20\,\mathrm{kHz}$로 돈다. 바깥 PD는 $1\,\mathrm{kHz}$로 돌고 $20\,\mathrm{rad/s}$, 즉 백 배 느리게 맞춘다. 그래서 바깥 루프는 안쪽 루프를 토크원으로 본다. 상자가 세운 전제를 넉넉히 채우는 셈이다.

그 전제가 무너지는 곳은 전압이다. 전류 루프는 $L\,di/dt$에 전압을 써야만 전류를 움직일 수 있고, 쓸 수 있는 것은 $V_s-Ri-k_en\dot\theta$다. $n=200$ 구동계에서 들어올리기의 중간 속도라면 역기전력만으로 $24\,\mathrm{V}$ 중 $23.4\,\mathrm{V}$를 쓰므로, 전류 루프는 관절이 가장 빠를 때 정확히 권한을 잃는다. §8의 $V_s$ 표시 뒤에 있는 메커니즘이 이것이다. 속도 포화는 전류 루프가 전압을 다 쓴 것이다.
...
**동작은 RMS로 판정한다.** 맨 위의 그림에서 들어올리기의 고리는 길이의 상당 부분을 열 한계선 위에서 보내는데, 그래도 된다. 열은 $i^2$을 적분하므로, $\tau_{th}$에 비해 짧은 듀티 사이클 — 여기서는 일과 쉼이 되풀이되는 패턴이고, [[02-foundations/basic-circuits-electronics|0.6.2 §6]]의 PWM 스위칭 비율이 아니다 — 에서 열 한계선이 묶는 것은 제곱평균제곱근 전류, $I_{\text{rms}}=\sqrt{\tfrac1T\int_0^Ti^2\,dt}\le I_{\text{cont}}$다. 들어올리기의 요구를 쉬지 않고 되풀이하면 $I_{\text{rms}}=3.11\,\mathrm{A}$로 연속의 $98\%$이고, 들어올릴 때마다 $4.2\,\mathrm{s}$씩 유지를 붙이면 5초 사이클은 $2.57\,\mathrm{A}$로 떨어진다.

**현장의 규모에서.** 건설 트랙의 $20\,\mathrm{kg}$ 외장 패널 S1([[05-construction-robotics/site-engineering|2.5]])을 P2 말단에 들면 어깨에 $19.62+20\cdot9.81\cdot1=215.8\,\mathrm{N{\cdot}m}$가 필요하다([[02-foundations/basic-mechanics|0.6.1 §8]]). 이 구동계의 연속 $25.3\,\mathrm{N{\cdot}m}$의 $8.5$배이니, 더 긴 시연이 아니라 다른 급의 구동계가 필요하다.

이 페이지의 나머지는 $R$을 고정한다. 뜨거운 권선의 저항이 오르면 유지에 무엇이 더해지는지는 아래 메모가 보인다.

> [!note]- 더 깊이 · Deeper
> **저항은 열과 함께 움직인다.** 구리의 저항은 켈빈당 약 $0.39\%$ 오르므로([[02-foundations/basic-circuits-electronics|0.6.2 §2]]), 뜨거운 권선은 같은 전류에서 열을 더 만든다. 이것을 '대상으로 한 번 끝까지'에 넣으면 유지의 상승은 $\Delta T=R_{th}i^2R_0(1+\alpha\Delta T)$를 풀어 $\Delta T=60.15/(1-0.0039\cdot60.15)=78.6\,\mathrm{K}$가 된다. 여전히 예산 안이지만, 상승이 $R$을 고정한 추정보다 $31\%$ 크다.

### 7. 기어 달린 관절의 역구동성

사람이 손으로 팔을 움직일 수 있는지, 충돌이 얼마나 단단한지, 모터 전류를 관절 토크로 읽을 수 있는지는 모두 전동 장치의 성질 하나 — 출력을 미는 힘이 그것을 얼마나 쉽게 움직이는가 — 에 달려 있고, 이 절은 그것을 P2의 기어 달린 어깨에서 잰다.
...
> - **왜 중요한가**: 위의 세 질문에 답한다. 셋째는 항목들 뒤의 문단이 다룬다.
...
- **권선 단락, 팔을 놓는다.** 어떤 구동계는 꺼질 때 권선을 단락한다(동적 제동). 그러면 역기전력이 브리지의 환류 경로([[02-foundations/basic-circuits-electronics|0.6.2 §6]])로 제동 전류를 흘리고, 관절에서의 댐핑은 $n=100$에서 $n^2k_tk_e/R=100\,\mathrm{N{\cdot}m{\cdot}s/rad}$다. 자세에서 놓인 P2는 그 댐핑이 중력과 맞먹는 속도, $19.62/100=0.196\,\mathrm{rad/s}$ 곧 약 $11^\circ/\mathrm{s}$로 내려앉고(이상적인 기어박스, 짧은 과도 이후), $n=300$에서는 $0.0218\,\mathrm{rad/s}$다. 단락된 모터는 관절이 움직이는 동안에만 듣는 브레이크다. 팔을 늦출 뿐 세우지는 못한다.

**전류를 토크로 읽는다는 것의 의미.** 전류로 관절 토크를 추정하는 구동계, $\hat\tau=\eta nk_ti$는 회전자와 마찰 항을 없는 셈 치고 있다. 들어올리기의 최대 가속도 $4.51\,\mathrm{rad/s^2}$에서 링크에 필요한 것은 $35.5\,\mathrm{N{\cdot}m}$인데, 전류 기반 추정은 $n=100$에서 $39.1$로 $10\%$ 높고, $n=300$에서 $68.3$으로 $93\%$ 높다. 회전자의 $n^2J_m\ddot\theta$는 전류에는 들어 있고 링크에는 없기 때문이다. 같은 오차가 접촉 감지에도 들어간다. 힘 센서가 없는 팔은 이 관절 토크에서 말단 힘을 거꾸로 읽어 내므로([[04-robotics/modern-robotics/ch05-velocity-kinematics|MR 5장 §3]]), 모델이 회전자 항을 빼 주지 않으면 $n=300$에서 들어올리기의 최대 가속도가 어깨를 약 $33\,\mathrm{N{\cdot}m}$로 미는 힘으로 읽힌다. 실제로는 없는 밀기다. 감속비가 낮은 구동계는 이 오차를 작게 두고 유지 열로 값을 치른다. §6의 맞바꿈을 센싱 쪽에서 본 것이다. Wensing 외(2017)는 다리 로봇의 액추에이터를 이 선택 위에 짓는다. 힘 제어를 관절에 동위치(collocated)로 두고, 충돌 시의 역구동성을 아래 메모의 impact mitigation factor로 잰다. P2 같은 고정 베이스 팔에서는 이 지표가 아무것도 말해 주지 않는다. [[04-robotics/force-compliance-control|13. §2]]의 토크 기반 임피던스 제어를 돕는 것이 바로 이런 구동계 — 역구동 가능한 관절 위의 반응 빠른 토크 인터페이스 — 이고, 그 페이지의 어드미턴스 쪽 가지는 위치나 속도 명령만 내주는 로봇을 위해 쓰인 것이다.
...
> $$\xi=\det\!\big(I-\Lambda\,\Lambda_L^{-1}\big),\qquad 0\le\xi\le1$$
>
> $\xi=1$이면 자유 동역학이 충격량 전부를 덜어 내고, $\Lambda\to\Lambda_L$이면 $\xi\to0$, 곧 관절이 잠긴 것처럼 군다. $\Lambda$와 $\Lambda_L$은 [[02-foundations/manipulator-kinematics-dynamics|기초 10 §6]]의 뜻의 작업공간 관성으로, 접촉 방향들 위의 행렬이고 방향이 하나라면 그냥 질량이다.
...
과제는 들어올리기다. 엘보를 $90^\circ$에 붙잡은 P2의 어깨를 $\theta_1=-0.5\,\mathrm{rad}$에서 고정 자세 $\theta_1=0$까지 $T=0.8\,\mathrm{s}$의 5차 시간 스케일링으로 올리고([[04-robotics/modern-robotics/ch09-trajectory-generation|MR 9장 §1]]; 최대 속도 $1.875\,\Delta\theta/T=1.17\,\mathrm{rad/s}$, 최대 가속도 $5.77\,\Delta\theta/T^2=4.51\,\mathrm{rad/s^2}$), 그다음 붙잡는다. 이 들어올리기는 [[04-robotics/modern-robotics/ch09-trajectory-generation|MR 9장]]이 P2에 고정한 액추에이터 한계($0.8\,\mathrm{rad/s}$, $2\,\mathrm{rad/s^2}$)보다 일부러 빠르다. 그 한계는 계획기가 그 안에 머무는 사양이고, 이 랩은 특정 구동계 하나가 그 너머에서 무엇을 할 수 있는지를 묻는다. $n=100$에서는 여유가 있고, $n=200$에서는 최대 속도 $1.17\,\mathrm{rad/s}$가 전압선에 닿는다. 제어기는 §5의 캐스케이드다. 중력 피드포워드가 있는 $1\,\mathrm{kHz}$ PD 위치 루프가 이상 기어 단위로 토크를 요구하므로 피드포워드는 $g_1/\eta$이고, 한 번 $n=100$에 맞춰 — $J_{eq}=4.75$에서 $20\,\mathrm{rad/s}$, 임계 감쇠 — 설계한 뒤 모든 감속비에 그대로 둔다. 한 기어박스에 맞춘 제어기가 그렇듯이. 그 안에서 $V_s$와 $I_{\max}$ 한계를 가진 $20\,\mathrm{kHz}$ PI 전류 루프가 돈다. 플랜트(제어되는 시스템)는 §1–§2다. 권선 방정식은 명시적 오일러로, 관절은 반암시적 오일러로 전진하고([[02-foundations/lab-kernel|0.7 §2와 §3]]), 둘 다 전류 루프의 $50\,\mu\mathrm{s}$로 돈다. 속도는 완벽하게 잰다고 두고 PWM은 평균 전압으로 바꿨는데, 둘 다 루프에 유리한 이상화다.
...
논문의 액추에이터 주장은 대개 그것을 참으로 만드는 조건 — 지속 시간, 자세, 공급 전압, 일률이 흐르는 방향 — 을 빼고 적은 숫자 하나다. 그래서 그것을 읽는다는 것은 행마다 그 조건을 묻는 일이다.
...
2. 다른 모터의 데이터시트가 속도 상수를 $191\,\mathrm{rpm/V}$로 준다. $\mathrm{N{\cdot}m/A}$ 단위의 토크 상수는 얼마이고, 왜 두 번째 측정이 필요 없는가?
3. 전류 루프는 팔이 없는 것처럼, 위치 루프는 전류 루프가 즉각인 것처럼 설계해도 되는 이유는?
4. 시연에서 관절이 무거운 도구를 팔을 다 뻗은 채 10초 동안 들고 있다. 그 관절이 하루 종일 그렇게 할 수 있다고 믿기 전에 물어볼 숫자 하나는?
...
> 2. 뒤집고 단위를 바꾼다. $191\,\mathrm{rpm/V}$는 볼트당 $191\cdot2\pi/60=20.0\,\mathrm{rad/s}$이므로 $k_e=1/20.0=0.050\,\mathrm{V{\cdot}s/rad}$이고, SI에서 토크 상수는 같은 숫자 $0.050\,\mathrm{N{\cdot}m/A}$다. 이 페이지 모터의 절반이다. 두 번째 측정이 필요 없는 것은 $k_t=k_e$가 모터 하나의 성질이 아니라 일률 보존 $e\,i=\tau_m\omega_m$의 진술이기 때문이다.
> 3. 시간 척도가 멀리 떨어져 있기 때문이다. 권선은 $\tau_e=L/R=1\,\mathrm{ms}$, 닫힌 전류 루프는 $0.5\,\mathrm{ms}$에 자리 잡는데, 속도는 회전자만의 $10\,\mathrm{ms}$보다 빨리 자리 잡을 수 없고 위치 루프는 전류 루프의 $2000$에 대해 $20\,\mathrm{rad/s}$로 맞춘다. 각 루프는 상대를 얼어 있거나 즉각인 것으로 본다. 그 전제는 전압이 바닥날 때 무너진다.
...
1. **그리기.** 위의 그림을 $n=50$ 구동계로. 전류 한계, 양 끝을 가진 전압선, 열 한계선, 전류를 적은 유지점 H, 그리고 들어올리기의 고리. 고리의 최대 요구는 최대 가속도 $4.51\,\mathrm{rad/s^2}$에서 $(M_{11}+\eta n^2J_m)\ddot\theta+g_1$이고, 그 순간 $g_1=21.94\,\mathrm{N{\cdot}m}$다. 유지와 들어올리기를 각각 어느 경계가 묶는지, 그리고 주변 온도에서 출발한 유지가 얼마나 가는지 말한다.
2. **유도.** (a) 어깨의 중력 토크가 가장 큰 자세 $\theta=(-26.57^\circ,\,90^\circ)$, $g_1=9.81\sqrt5=21.94\,\mathrm{N{\cdot}m}$($2\cos\theta_1-\sin\theta_1$을 미분해 찾는다)의 P2를 $n=100$ 구동계로: 모터 토크, 전류, 전압, 열, 정상 상승. 무기한 붙잡는가? (b) 그 자세를 연속으로 붙잡는 가장 작은 정수 감속비. (c) $n=160$에서, 두 구동계를 단 $\theta_2=90^\circ$의 질량 행렬과 결합 $M_{12}/\sqrt{M_{11}M_{22}}$. (d) §4는 중력을 빼고 관절 가속도 한 단위당 모터 토크를 최소로 하는 감속비 $n^\ast$를 $\theta_2=90^\circ$의 어깨에 대해 유도했다. 그것을 엘보($M_{22}=1\,\mathrm{kg{\cdot}m^2}$, 어깨는 붙잡음), 팔을 편 어깨($\theta_2=0^\circ$, $M_{11}=5$), 접은 어깨($\theta_2=180^\circ$, $M_{11}=1$)에 대해 구하라. 감속비 하나가 모든 자세에 맞을 수 있는가? §4의 $n^\ast=193.6$은 편 자세와 접은 자세에서 각 자세의 최적에 비해 얼마를 치르는가? (e) $n=200$에서의 기계적 시정수와 $\tau_e$에 대한 비.
...
> - **$n$에 대한 비례를 확인한다.** 전류 한계는 $n$에 비례해 오르고, 무부하 속도는 $1/n$로 떨어지고, 전압선의 기울기는 $n^2$로 가팔라진다(§3). 그러니 감속비를 절반으로 하면 영역은 높이가 절반, 폭이 두 배여야 한다.

> [!tip]- 정답 · Solutions
> 1. 전류 한계 $\eta nk_tI_{\max}=0.8\cdot50\cdot0.1\cdot10=40\,\mathrm{N{\cdot}m}$. 전압선 $\tau_j=(0.8\cdot50\cdot0.1/1)(24-0.1\cdot50\,\dot\theta)=96-20\,\dot\theta$, 모서리 $(96-40)/20=2.8\,\mathrm{rad/s}$에서 무부하 속도 $4.8\,\mathrm{rad/s}$까지. 열 한계선 $0.8\cdot50\cdot0.1\cdot3.162=12.65\,\mathrm{N{\cdot}m}$, 전압선과 $4.17\,\mathrm{rad/s}$에서 만난다. 영역은 $n=100$보다 높이가 절반, 폭이 두 배다. H는 $19.62\,\mathrm{N{\cdot}m}$에 이제 $19.62/(0.8\cdot50\cdot0.1)=4.905\,\mathrm{A}$가 들고 열 한계선 *위*에 있으므로, 유지는 열이 묶는다. 주변 온도에서 출발하면 $-60\ln(1-100/240.6)=32.2\,\mathrm{s}$ 간다. 들어올리기는 들어간다. 최대 요구가 $(3+0.8\cdot50^2\cdot10^{-4})\cdot4.51+21.94=3.2\cdot4.51+21.94=36.4\,\mathrm{N{\cdot}m}$로 $40\,\mathrm{N{\cdot}m}$ 전류 한계 아래이고, 동작 중간의 요구 $1.17\,\mathrm{rad/s}$, $21.5\,\mathrm{N{\cdot}m}$에서 전압선은 $96-20\cdot1.17=72.6$을 주며 그것도 $40$에서 막힌다. $n=50$에서는 전압이 아니라 열이 묶는다. 동작을 쉽게 만드는 감속비가 하루 종일의 유지는 불가능하게 만든다 — §8 창의 아래쪽 끝이다.
> 2. (a) $\tau_m=21.94/(0.8\cdot100)=0.2742\,\mathrm{N{\cdot}m}$, $i=2.742\,\mathrm{A}$, $V=Ri=2.74\,\mathrm{V}$, $P=i^2R=7.52\,\mathrm{W}$, $\Delta T=R_{th}P=75.2\,\mathrm{K}$. $100\,\mathrm{K}$ 아래이므로 무기한 붙잡고, 열 예산의 $75\%$를 쓴다. (b) $n\ge21.94/(0.8\cdot0.1\cdot3.162)=86.7$이므로 $n=87$. (c) $n^2J_m=2.56$, $nJ_m=0.016$이므로 $M=\begin{pmatrix}5.5601&1.016\\1.016&3.56\end{pmatrix}$이고 결합은 $1.016/\sqrt{5.5601\cdot3.56}=0.228$, 맨 팔의 $0.577$에 대해서다. (d) 최적은 관절이 모는 관성 $M$이 무엇이든 $n^\ast=\sqrt{M/(\eta J_m)}$이므로, 엘보는 $n^\ast=\sqrt{1/(0.8\cdot10^{-4})}=111.8$ — 고정한 $n=100$은 그 최솟값에서 $0.6\%$ 안에 있다 — 편 팔은 $\sqrt{5/(0.8\cdot10^{-4})}=250$, 접은 팔은 다시 $111.8$이다. $M_{11}=3+2\cos\theta_2$가 엘보와 함께 움직이므로 감속비 하나가 모든 자세에 맞을 수는 없다. 다만 최솟값은 얕다. $n=193.6$에서 가속도 한 단위당 모터 토크 $nJ_m+M/(\eta n)$은 편 팔에서 $0.0516$으로 그 최적 $2\sqrt{J_mM/\eta}=0.0500\,\mathrm{N{\cdot}m}$(rad/s²당)보다 $3.3\%$ 많고, 접은 팔에서 $0.0258$로 그 최적 $0.0224$보다 $15.5\%$ 많다. §4가 감속비를 관성 정합이 아니라 중력과 속도에 맡기는 이유다. (e) $J=10^{-4}+3/(0.8\cdot4\cdot10^4)=1.9375\times10^{-4}\,\mathrm{kg{\cdot}m^2}$이므로 $\tau_{\text{mech}}=JR/(k_tk_e)=19.4\,\mathrm{ms}$, $\tau_e$의 $19$배 — 열 배라는 바닥에서 여전히 멀다.
