...
`terse-sections.json`의 04·02 항목 상당수는 **표로 설계된 절**이다(방법 계열 표, 주장 읽기 표, 용어 표). 그런 절에는 손대지 마라. 아래 열거된 것만 한다.

**2026-09-10 정정.** 이 문단은 원래 "대부분 표로 설계된 절"이라고 단정했는데, 실제로 재보니 남은 136개 중 표가 있는 것은 55개뿐이다(불릿 목록 26개, 표도 불릿도 없는 순수 산문 55개). 순수 산문 55개는 양 언어로 전부 정독했고 실제 결함은 3개였다 — 정의 없이 쓰인 기호 $f_c$, 한국어 반쪽에만 있던 단위·프레임 문장, 정의 없이 쓰인 스크류 피치 $h$. 셋 다 고쳤다. 나머지 52개는 짧지만 완결이었다. **결론: 짧다는 것은 결함 신호가 아니다.** 단어 수를 보고 손대지 말고, 기호가 정의 없이 쓰였는지·양 언어 반쪽 내용이 다른지를 봐라.
