...
## 6. 놓쳤던 통합 서베이와 그 처리 (2026-09-03)

사용자가 https://gisbi-kim.github.io/vla-unified-survey-2024-2026/ 를 지목했다. §1의 조사가 이 페이지를 놓쳤다.

**놓친 이유**: 사이트맵에도 RSS에도 없고 다른 글에서 링크되지도 않는다. RSS + 사이트맵 + 링크 추적으로는 닿을 수 없었다. 더 큰 잘못은 그 다음이다. "VLA 통합 서베이"라는 사용자의 말이 내가 찾은 `/vla-runtime-harnessing/`을 가리킨다고 단정했다. 못 찾았다고 말할 자리에서 추측했다. 이후의 VLA 평가가 전부 다른 문서에 대한 것이 되었다.

**그 문서의 성격**: 약 105,000 단어, 고유 arXiv 인용 155개, 서베이 10여 편의 종합. 인용 10개를 무작위로 arXiv 조회했고 전부 실재하며 제목도 맞았다. 본문 사실 표본(CLIP 4억 쌍, ViT 16×16, RT-2 55B, SmolVLA 450M)도 맞았다. `/vla-runtime-harnessing/`(주장문, 4,800단어)과는 다른 물건이다.

**처리 원칙**: 페이지를 인용하지 않는다. 분류 체계는 사실이 아니라 배열이므로 출처를 댈 대상이 아니고, 그 축을 참고하되 모든 주장은 1차 서베이 원문에서 확인한 것만 쓴다. 그래서 이 문서는 무엇을 읽을지 고르는 데만 썼고 위키에는 남기지 않았다.

**결과물**: `notes/4-vla/index.md`를 여덟 줄 스텁에서 축 있는 지도로 재작성했다(커밋 "Give the VLA notes a map with axes instead of a date order"). 근거는 전부 초록에서 직접 확인했다.

| 축 | 1차 출처 |
|---|---|
| 액션 토큰 8종과 그것이 첫 번째 설계 선택이라는 주장 | Chen 외 arXiv:2507.01925 |
| 구간 나누기가 고주파·정교 과제에서 무너진다 / DCT 압축 토큰화 / 실로봇 궤적 100만개 | FAST arXiv:2501.09747 |
| 서베이 다섯 편의 서로 다른 투영축 | 2405.14093, 2507.01925, 2509.19012, 2510.24795, 2512.11362 각 초록 |
| 효율성 세 기둥(모델 설계·학습·데이터 수집) | Yu 외 arXiv:2510.24795 |
| GPU 한 장 학습, 커뮤니티 수집 데이터, 수십억 파라미터 VLA와 대조 | SmolVLA arXiv:2506.01844 |

**직접 계산한 것**: 우리 정전 12편을 Chen의 8축에 대 보니 여덟 중 다섯 종류에 논문이 아예 없다. 표를 세어 확인했다(EN/KR 각 8행, '없음' 5행). 처음에 여섯이라고 썼다가 재검산해서 고쳤다.
