const COLORS={"Deep Learning":"#396a93","Generative Models":"#775a9e","Physical AI":"#1d6b55","Robot Learning":"#d7753f","Computer Vision":"#b1912f","Robotics":"#5f7f72","Construction Physical AI":"#a64f58"}
...
fetch("./data.json").then(r=>{if(!r.ok)throw Error("dataset unavailable");return r.json()}).then(data=>{
  DATA=data
...
function buildFilters(){
  const groups=["All",...new Set(DATA.topics.map(t=>t.group))]
  ...
  $("#depthFilter").onchange=render;$("#statusFilter").onchange=render
}
function filtered(){
  const depth=$("#depthFilter").value,status=$("#statusFilter").value
  return DATA.topics.filter(t=>(scope==="All"||t.group===scope)&&(depth==="All"||depth==="Mastery"||t.depth===depth)&&(status==="All"||t.status===status))
}
function render(){
...
function renderRadar(topics){
  ...
  topics.forEach(t=>{
    ...
    const circle=svg("circle",{cx:x,cy:y,r,fill:COLORS[t.group]||"#6b7b73",opacity:t.confidence==="Early"?.48:.82,class:`bubble ${selected?.id===t.id?"selected":""}`})
    ...
    root.append(circle)
    if(scaleLeaders.has(t.id)||t.status==="Fast Rising"||t.status==="Emerging"){
...
function renderDetail(t){
  const mode=$("#depthFilter").value
  const depthText=mode==="Mastery"?"Mastery only if this becomes the thesis contribution":`${t.depth} minimum`
  $("#detail").innerHTML=`<p class="kicker">EVIDENCE</p><h2>${esc(t.label)}</h2><span class="status">${esc(t.status)}</span><span class="status">${esc(t.depth)} target</span>
  ...
  <p><b>Study guidance</b><br>${esc(depthText)}. Adjacent fields need accurate literacy; implementation choices need working knowledge.</p><p class="hint">Matched aliases: ${(t.aliases||[]).map(esc).join(" · ")}</p><p><b>Recent venue spread</b><br>${t.venues.length?t.venues.map(esc).join(" · "):"No multi-venue evidence yet"}</p>
...
function renderMethod(){
  ...
  $("#methodBody").innerHTML=`<p><b>Included evidence:</b> papers indexed in named peer-reviewed proceedings. arXiv and workshops are excluded. Current coverage: ${coverage}/${DATA.audit.length} venue-years, ${DATA.paperCount.toLocaleString()} papers.</p><p><b>Topic assignment:</b> transparent, multi-label title patterns. This favors precision and auditability over recall; it can miss papers whose title omits the method. Counts are normalized per 1,000 papers per year before momentum is calculated.</p><p><b>Trend evidence:</b> recent volume + normalized five-year slope + burst + venue breadth, shrunk toward zero for small samples. Status is a navigation aid, not a prediction of scientific value.</p><p><b>Known limits:</b> DBLP indexing can lag; institutions and abstracts are not yet used; technical debates require human review. Missing venue-years: ${missing.length}. Every result exposes aliases, counts, venues, and representative papers so it can be challenged.</p>`
}
