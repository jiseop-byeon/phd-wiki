...
function render(){
  ...
  $("#metrics").innerHTML=[
    [topics.length,"visible topics"],[topics.reduce((a,t)=>a+t.recentVolume,0),"topic–paper matches · recent (duplicates possible)"],
  ...
  if(!topics.length){
    selected=null
    $("#radar").innerHTML="<text x='24' y='40' class='tick'>No topics match the current filters.</text>"
    ;["rankEstablished","rankRising","rankEmerging"].forEach(id=>$("#"+id).innerHTML="")
    $("#timelineTitle").textContent="No matching topic";$("#confidenceBadge").textContent=""
    $("#timeline").innerHTML="";$("#detail").innerHTML="<h2>No matching topic</h2><p>Change the scope or status filter.</p>"
    return
  }
  renderRadar(topics);renderRanks(topics)
...
function renderTimeline(t){
  $("#timelineTitle").textContent=`${t.label} · topic share per 1,000 papers`;$("#confidenceBadge").textContent=`${t.confidence} evidence support`
  ...
  pts.forEach((pt,i)=>{root.append(svg("circle",{cx:pt[0],cy:pt[1],r:5,class:"dot"}));const tx=svg("text",{x:pt[0],y:H-14,"text-anchor":"middle",class:"tick"});tx.textContent=DATA.years[i];root.append(tx);const val=svg("text",{x:pt[0],y:pt[1]-10,"text-anchor":"middle",class:"tick"});val.textContent=t.shares[i].toFixed(1);root.append(val)})
}
function renderDetail(t){
  $("#detail").innerHTML=`<p class="kicker">EVIDENCE</p><h2>${esc(t.label)}</h2><span class="status">${esc(t.status)}</span>
  <div class="detail-grid"><div class="detail-stat"><b>${t.recentVolume}</b><span>2024–25 topic matches</span></div><div class="detail-stat"><b>${t.momentum>0?"+":""}${t.momentum}</b><span>share momentum</span></div><div class="detail-stat"><b>${t.breadth}</b><span>recent venues</span></div><div class="detail-stat"><b>${t.trendScore}</b><span>trend evidence</span></div><div class="detail-stat"><b>${t.support}</b><span>five-year support</span></div><div class="detail-stat"><b>${t.confidence}</b><span>evidence support</span></div></div>
  ...
  <h3>Recent title-matched paper examples</h3><ul class="paper-list">${t.papers.length?t.papers.map(p=>`<li><a href="${esc(p.url)}" target="_blank" rel="noreferrer">${esc(p.title)}</a><small>${esc(p.venue)} · ${p.year}${p.authors?.length?" · "+esc(p.authors.slice(0,3).join(", ")):""}</small></li>`).join(""):"<li>No paper example in the current title-matched sample.</li>"}</ul>`
}
